...
x008012_g_DeliveryScript = 006668
x008012_g_DeliveryList = {1010237}
--**********************************
--事件列表
--**********************************
function x008012_UpdateEventList( sceneId, selfId,targetId )
    ...
    AddText(sceneId,"  不要和我说废话，老老实实完成你的试炼吧。")
		--配表任务
	for i,Deliveryidx in x008012_g_DeliveryList do
		CallScriptFunction( x008012_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
...
function x008012_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x008012_g_DeliveryList do
		if eventId == x008012_g_DeliveryScript then
...
function x008012_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x008012_g_DeliveryList do
		if missionScriptId == x008012_g_DeliveryScript then
