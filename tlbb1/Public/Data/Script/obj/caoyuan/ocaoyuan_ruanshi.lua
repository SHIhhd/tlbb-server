...
x020002_g_KillMonsterScript = 006666
x020002_g_KillMonsterList = {1000080,1000083}
--**********************************
--事件列表
--**********************************
function x020002_UpdateEventList( sceneId, selfId,targetId )
	...
	AddText(sceneId,"  你就是大侠"..PlayerName.."#{OBJ_caoyuan_0003}")
	for i,KillMonsteridx in x020002_g_KillMonsterList do
		CallScriptFunction( x020002_g_KillMonsterScript, "OnEnumerate",sceneId, selfId, targetId,KillMonsteridx)
	end
...
function x020002_OnEventRequest( sceneId, selfId, targetId, eventId )
	for i,KillMonsteridx in x020002_g_KillMonsterList do
		if eventId == x020002_g_KillMonsterScript then
...
function x020002_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	local ret = 0
	for i,KillMonsteridx in x020002_g_KillMonsterList do
		if missionScriptId == x020002_g_KillMonsterScript then
