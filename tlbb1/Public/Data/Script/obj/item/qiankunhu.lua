...
--效果的ID
x300054_g_Impact = 57 --临时写这个
x300054_g_ImpactMutexOther = 343
...
function x300054_OnConditionCheck( sceneId, selfId )
	...
	local checkRet = LuaFnHaveImpactOfSpecificMutexID(sceneId, selfId, x300054_g_ImpactMutexOther)
	if checkRet and checkRet == 1 then
		x300054_NotifyTip(sceneId, selfId, "只有等自动捡取物品效果消失之后才能使用。");
		return 0
	end
...
function x300054_OnActivateOnce( sceneId, selfId )
	...
	if checkRet and checkRet == 1 then
		...
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x300054_g_Impact, 0);
	end
	return 1;
end
