...
x300022_g_SceneMapDefine = {	
										...
										{sceneId=32,	sceneName="玉溪",	CorpseMonsterId=3516},
...
function x300022_DiscoverItem(sceneId, selfId)
		...
		strText = format("#W#{_INFOUSR%s}#cff99cc在#G%s#cff99cc挖宝时幸运的得到了#W#{_INFOMSG%s}", PlayName, sceneName, ItemName)
		--PrintNum(bBroadCast)
...
function x300022_DiscoverCorpseMonster(sceneId, selfId)
		...
		local strText = format("#W#{_INFOUSR%s}#cff99cc在挖宝时不慎泄漏消息，在#G%s#cff99cc引来了一伙#{_BOSS48}。江湖志士只要去剿灭马贼，就能获得马贼的藏宝！", 
...
function x300022_OnDefaultEvent( sceneId, selfId, BagPos)
	...
	local strText = format("你到%s的#{_INFOAIM%d,%d,%d,-1}#W才能使用此物。", sceneName, targetX, targetZ,targetSceneId)
	--取得玩家当前坐标
	...
	if(targetSceneId == sceneId) then
		--地图上闪烁位置
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, targetX, targetZ, "藏宝图" )
		x300022_NotifyFailTips( sceneId, selfId, "地图上闪烁的地方就是藏宝图入口奥" )
	end

	if targetSceneId ~= sceneId or Distance > r then
			BeginEvent(sceneId)
				AddText(sceneId,strText)
				AddText(sceneId, "#G小提示：#W#r   点位在地图上有#Y闪烁#W奥。#r   #e000000有的时候藏宝图会出现在#gfff0f0高于您目前等级的地图#g000000这些地图上面怪物等级较高，#gfff0f0请千万小心#g000000，您可以将藏宝图出售给其他玩家或者留待自己等级上升之后再来使用。")
			EndEvent( )
			DispatchEventList(sceneId,selfId,-1)
		return
	end	
	...
	--elseif ret < 6 then --获得物品
		...
		--	return
		--end
	--elseif ret < 97 then --遇盗墓贼
		--x300022_DiscoverCorpseMonster(sceneId, selfId)
	elseif ret < 45 then --掉入宝藏
...
-- 屏幕中间信息提示
--**********************************
function x300022_NotifyFailTips( sceneId, selfId, Tip )
	BeginEvent( sceneId )
		AddText( sceneId, Tip )
	EndEvent( sceneId )
	DispatchMissionTips( sceneId, selfId )
