...
function x300089_OnDefaultEvent( sceneId, selfId, bagIndex )
	...
	if HaveKfsEquiped(sceneId,selfId) ~= 1 then
		x300089_NotifyTip( sceneId, selfId, "#{WH_090729_08}")
		return 0
	end
	local itemTblIndex = LuaFnGetItemTableIndexByIndex(sceneId,selfId,bagIndex)
	if x300089_g_SoulExp[itemTblIndex] ==nil then
		x300089_NotifyTip( sceneId, selfId, "物品数据非法")
		return 0
	end
	--提示在底层走就行了，免得反复导出
	local nRet = LuaFnAddEquipedKfsExp(sceneId,selfId,x300089_g_SoulExp[itemTblIndex]);
	if nRet ~= 1 then
		return 0;
	end
	--扣除物品
...
--	x300089_NotifyTip( sceneId, selfId, "x300089_OnConditionCheck")
	--校验使用的物品
...
--屏幕中间提示：
--**********************************
function x300089_NotifyTip( sceneId, selfId, Tips)
	BeginEvent( sceneId )
