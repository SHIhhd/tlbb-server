...
function x300100_OnActivateOnce( sceneId, selfId )
	...
	if (itemTblIndex == 30309186) or (itemTblIndex == 30309187) then --如果是还童丹：鸳鸯
		strText = format("#{HT10}#{_INFOUSR%s}#{HT11}#{_ITEMINFO%d}#{HT12}#{_INFOMSG%s}#{HT13}", PlayerName, itemTblIndex, PetName);	
	else
		local inforand = random(3)		
		if inforand == 1 then
			strText = format("#{HT01}#{_ITEMINFO%d}#{HT02}#{_INFOMSG%s}#{HT03}#{_INFOUSR%s}#{HT04}", itemTblIndex, PetName, PlayerName )
		elseif inforand == 2 then
			strText = format("#{HT05}#{_INFOUSR%s}#{HT06}#{_ITEMINFO%d}#{HT07}#{_INFOMSG%s}#H#{HSLJJF_2}", PlayerName, itemTblIndex, PetName )
		else
			strText = format("#{_INFOUSR%s}#{HT08}#{_ITEMINFO%d}#{HT09}#{_INFOMSG%s}#H#{HSLJJF_2}", PlayerName, itemTblIndex, PetName )
		end
	end
