...
--#w【队长名】#P在燕子坞调兵遣将，运筹帷幄，在使用了瞒天过海，暗度陈仓等三十六计之后，打得#{_BOSS0}只得使用第三十七计逃之夭夭了。
	...
	for i=0, nearteammembercount-1  do
		...
		if LuaFnIsObjValid(sceneId,nPlayerId) == 1 and LuaFnIsCanDoScriptLogic(sceneId,nPlayerId) == 1 then
			--周活跃数据埋点
			LuaFnFinishZhouHuoYueMission(sceneId,nPlayerId,12)
			--英雄大会埋点
			if LuaFnHaveImpactOfSpecificDataIndex(sceneId,nPlayerId,10541) == 1 then
				AddHerosReturnsRecoed(sceneId,nPlayerId,12)
			end
			CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, nPlayerId,CAMPAIGN_YANZIWU)
			--云侠赴战
			CallScriptFunction(890289,"OnFinish",sceneId,nPlayerId,CAMPAIGN_YANZIWU)
		end
	end	
	--高光时刻结算
