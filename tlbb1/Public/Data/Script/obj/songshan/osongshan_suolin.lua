...
x003008_g_DeliveryScript = 006668
x003008_g_DeliveryList = {1010232}
--**********************************
--事件列表
--**********************************
function x003008_UpdateEventList( sceneId, selfId,targetId )
    ...
    AddText(sceneId,"  这里就是嵩山了吧，你知道嵩阳书院在哪里吗？")
	--配表任务
	for i,Deliveryidx in x003008_g_DeliveryList do
		CallScriptFunction( x003008_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
...
function x003008_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x003008_g_DeliveryList do
		if eventId == x003008_g_DeliveryScript then
...
function x003008_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x003008_g_DeliveryList do
		if missionScriptId == x003008_g_DeliveryScript then
