...
x003004_g_DeliveryScript = 006668
x003004_g_DeliveryList = {1010032}
x003004_g_LootItemscript = 006667
x003004_g_LootItemList = {1030031,1030032}
x003004_g_KillMonsterScript = 006666
x003004_g_KillMonsterList = {1009401}
--**********************************
--事件列表
--**********************************
function x003004_UpdateEventList( sceneId, selfId,targetId )
	...
	--配表任务
	for i,Deliveryidx in x003004_g_DeliveryList do
		CallScriptFunction( x003004_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
	for i, LootItemidx in x003004_g_LootItemList do
		CallScriptFunction( x003004_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
	end
	for i,KillMonsteridx in x003004_g_KillMonsterList do
		CallScriptFunction( x003004_g_KillMonsterScript, "OnEnumerate",sceneId, selfId, targetId,KillMonsteridx)
	end
...
function x003004_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x003004_g_DeliveryList do
		if eventId == x003004_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x003004_g_LootItemList do
		if eventId == x003004_g_LootItemscript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,LootItemidx)
		end
	end
	for i,KillMonsteridx in x003004_g_KillMonsterList do
		if eventId == x003004_g_KillMonsterScript then
...
function x003004_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x003004_g_DeliveryList do
		if missionScriptId == x003004_g_DeliveryScript then
	...
	for i, LootItemidx in x003004_g_LootItemList do
		if missionScriptId == x003004_g_LootItemscript then
	...
	for i,KillMonsteridx in x003004_g_KillMonsterList do
		if missionScriptId == x003004_g_KillMonsterScript then
