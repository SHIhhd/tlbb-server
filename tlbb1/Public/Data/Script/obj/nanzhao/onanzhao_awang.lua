...
x028001_g_EnterareaScript = 006669
x028001_g_EnterareaList = {1020060}
x028001_g_KillMonsterScript = 006666
x028001_g_KillMonsterList = {1000161}
--**********************************
--事件列表
--**********************************
function x028001_UpdateEventList( sceneId, selfId,targetId )
	...
	AddText(sceneId,"我是阿旺")
	for i,Enterareaidx in x028001_g_EnterareaList do
		CallScriptFunction( x028001_g_EnterareaScript, "OnEnumerate",sceneId, selfId, targetId ,Enterareaidx)
	end
	for i,KillMonsteridx in x028001_g_KillMonsterList do
		CallScriptFunction( x028001_g_KillMonsterScript, "OnEnumerate",sceneId, selfId, targetId,KillMonsteridx)
	end
...
function x028001_OnEventRequest( sceneId, selfId, targetId, eventId )
	for i,Enterareaidx in x028001_g_EnterareaList do
		if eventId == x028001_g_EnterareaScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId,Enterareaidx)
		end
	end
	for i,KillMonsteridx in x028001_g_KillMonsterList do
		if eventId == x028001_g_KillMonsterScript then
...
function x028001_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	local ret = 0
	for i, Enterareaidx in x028001_g_EnterareaList do
		if missionScriptId == x028001_g_EnterareaScript then
	...
	for i,KillMonsteridx in x028001_g_KillMonsterList do
		if missionScriptId == x028001_g_KillMonsterScript then
