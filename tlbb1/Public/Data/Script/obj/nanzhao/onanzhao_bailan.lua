...
x028003_g_DeliveryScript = 006668
x028003_g_DeliveryList = {1010161}
x028003_g_LootItemscript = 006667
x028003_g_LootItemList = {1030161}
x028003_g_KillMonsterScript = 006666
x028003_g_KillMonsterList = {1000163}
--**********************************
--事件列表
--**********************************
function x028003_UpdateEventList( sceneId, selfId,targetId )
	...
	AddText(sceneId,"我是白兰")
	for i, Deliveryidx in x028003_g_DeliveryList do
		CallScriptFunction( x028003_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
	for i, LootItemidx in x028003_g_LootItemList do
		CallScriptFunction( x028003_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
	end
	for i,KillMonsteridx in x028003_g_KillMonsterList do
		CallScriptFunction( x028003_g_KillMonsterScript, "OnEnumerate",sceneId, selfId, targetId,KillMonsteridx)
	end
...
function x028003_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x028003_g_DeliveryList do
		if eventId == x028003_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x028003_g_LootItemList do
		if eventId == x028003_g_LootItemscript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,LootItemidx)
		end
	end
	for i,KillMonsteridx in x028003_g_KillMonsterList do
		if eventId == x028003_g_KillMonsterScript then
...
function x028003_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x028003_g_DeliveryList do
		if missionScriptId == x028003_g_DeliveryScript then
	...
	for i, LootItemidx in x028003_g_LootItemList do
		if missionScriptId == x028003_g_LootItemscript then
	...
	for i,KillMonsteridx in x028003_g_KillMonsterList do
		if missionScriptId == x028003_g_KillMonsterScript then
