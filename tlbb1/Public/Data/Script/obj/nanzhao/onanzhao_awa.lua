...
x028000_g_KillMonsterScript = 006666
x028000_g_KillMonsterList = {1000160}
--**********************************
--事件列表
--**********************************
function x028000_UpdateEventList( sceneId, selfId,targetId )
	...
	AddText(sceneId,"真怀念以前的家啊！")
	for i,KillMonsteridx in x028000_g_KillMonsterList do
		CallScriptFunction( x028000_g_KillMonsterScript, "OnEnumerate",sceneId, selfId, targetId,KillMonsteridx)
	end
...
function x028000_OnEventRequest( sceneId, selfId, targetId, eventId )
	for i,KillMonsteridx in x028000_g_KillMonsterList do
		if eventId == x028000_g_KillMonsterScript then
...
function x028000_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	local ret = 0
	for i,KillMonsteridx in x028000_g_KillMonsterList do
		if missionScriptId == x028000_g_KillMonsterScript then
