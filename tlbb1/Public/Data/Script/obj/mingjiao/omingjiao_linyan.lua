...
--所拥有的事件ID列表
x011013_g_eventList={229001,222000,222001,222002,222003,222004,222005,222006,222007,222008,222009,222010,222011,222012,222020,222900,050061,891279}
x011013_g_DeliveryScript = 006668
x011013_g_DeliveryList = {1010222,1018749,1018750,1018751,1018752,1018753,1018754,1018755,1018756,1018757,1018758}
x011013_g_LootItemscript = 006667
x011013_g_LootItemList = {1039805}
--**********************************
--事件交互入口
--**********************************
function x011013_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, Deliveryidx in x011013_g_DeliveryList do
			CallScriptFunction( x011013_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
		for i, LootItemidx in x011013_g_LootItemList do
			CallScriptFunction( x011013_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
		end
		...
		CallScriptFunction( 229013, "OnEnumerate",sceneId, selfId, targetId, MP_MINGJIAO )
...
function x011013_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif eventId == 229013 then
		CallScriptFunction( 229013, "OnDefaultEvent",sceneId, selfId, targetId, MP_MINGJIAO, GetNumText() )
		return
	...
	--配表任务
	for i,Deliveryidx in x011013_g_DeliveryList do
		if eventId == x011013_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x011013_g_LootItemList do
		if eventId == x011013_g_LootItemscript then
...
function x011013_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x011013_g_DeliveryList do
		if missionScriptId == x011013_g_DeliveryScript then
	...
	for i, LootItemidx in x011013_g_LootItemList do
		if missionScriptId == x011013_g_LootItemscript then
