...
x011003_g_mpInfo		= {}
x011003_g_mpInfo[0]	= { "星宿", 16,  96, 152, MP_XINGSU }
x011003_g_mpInfo[1]	= { "逍遥", 14,  67, 145, MP_XIAOYAO }
x011003_g_mpInfo[2]	= { "少林",  9,  95, 137, MP_SHAOLIN }
x011003_g_mpInfo[3]	= { "天山", 17,  95, 120, MP_TIANSHAN }
x011003_g_mpInfo[4]	= { "天龙", 13,  96, 120, MP_DALI }
x011003_g_mpInfo[5]	= { "峨嵋", 15,  89, 144, MP_EMEI }
x011003_g_mpInfo[6]	= { "武当", 12, 103, 140, MP_WUDANG }
x011003_g_mpInfo[7]	= { "明教", 11,  98, 167, MP_MINGJIAO }
x011003_g_mpInfo[8]	= { "丐帮", 10,  91, 116, MP_GAIBANG }
x011003_g_mpInfo[10] = { "曼陀", SCENE_MANTUOSHANZHUANG,  139, 162, MP_MANTUO }
x011003_g_mpInfo[11] = { "恶人谷", 504,  122, 151, MP_ERENGU }
...
function x011003_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if	GetLevel( sceneId, selfId)>=10  then	
			AddNumText(sceneId,x011003_g_scriptId,"洛阳",9,0)
			AddNumText(sceneId,x011003_g_scriptId,"苏州",9,1)
			AddNumText( sceneId, x011003_g_scriptId, "洛阳 - 九州商会", 9, 3 )
			AddNumText( sceneId, x011003_g_scriptId, "苏州 - 铁匠铺", 9, 4 )
		end
		
		--add by WTT
		if	GetLevel( sceneId, selfId)>=20  then	
			AddNumText( sceneId, x011003_g_scriptId, "束河古镇", 9, 6 )
		end
		
		if	GetLevel( sceneId, selfId)>=75  then	
			AddNumText( sceneId, x011003_g_scriptId, "#{MPCSLL_80925_01}", 9, 5 )
		end
		AddNumText(sceneId,x011003_g_scriptId,"大理",9,2)
		
    --[tx42913]
    CallScriptFunction( x011003_g_xuanWuDaoId, "OnEnumerate",sceneId, selfId, targetId )
		--[/tx42913]
		
		AddNumText(sceneId,x011003_g_scriptId,"带我去其他门派吧",9,11)

		-- 我怎样才能去敦煌和嵩山
		if GetLevel(sceneId,selfId) < 30 then
			AddNumText( sceneId, x011003_g_scriptId, "我怎样才能去敦煌和嵩山", 11, 2000 )
		end
	EndEvent(sceneId)
...
--[/tx42913]		
    ...
    if  GetNumText() == 11 then
		...
		BeginEvent(sceneId)
		    for i, mp in x011003_g_mpInfo do
				AddNumText( sceneId, x011003_g_scriptId, "门派 - "..mp[1], 9, i+12 )
			end	
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)	
		return 
