...
--所拥有的事件ID列表
x016010_g_eventList={229007,227000,227001,227002,227003,227004,227005,227006,227007,227008,227009,227010,227011,227012,227020,227900,050061,891279}
x016010_g_DeliveryScript = 006668
x016010_g_DeliveryList = {1010227,1018799,1018800,1018801,1018802,1018803,1018804,1018805,1018806,1018807,1018808}
x016010_g_LootItemscript = 006667
x016010_g_LootItemList = {1039809}
--**********************************
--事件交互入口
--**********************************
function x016010_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, Deliveryidx in x016010_g_DeliveryList do
			CallScriptFunction( x016010_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
		for i, LootItemidx in x016010_g_LootItemList do
			CallScriptFunction( x016010_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
		end
		...
		CallScriptFunction( 229013, "OnEnumerate",sceneId, selfId, targetId, MP_XINGSU )
...
function x016010_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif eventId == 229013 then
		CallScriptFunction( 229013, "OnDefaultEvent",sceneId, selfId, targetId, MP_XINGSU, GetNumText() )
		return
	...
	--配表任务
	for i,Deliveryidx in x016010_g_DeliveryList do
		if eventId == x016010_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x016010_g_LootItemList do
		if eventId == x016010_g_LootItemscript then
...
function x016010_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x016010_g_DeliveryList do
		if missionScriptId == x016010_g_DeliveryScript then
	...
	for i, LootItemidx in x016010_g_LootItemList do
		if missionScriptId == x016010_g_LootItemscript then
