...
x030007_g_DeliveryScript = 006668
x030007_g_DeliveryList = {1010234}
--**********************************
--事件列表
--**********************************
function x030007_UpdateEventList( sceneId, selfId,targetId )
	...
	for i, Deliveryidx in x030007_g_DeliveryList do
		CallScriptFunction( x030007_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
...
function x030007_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x030007_g_DeliveryList do
		if eventId == x030007_g_DeliveryScript then
...
function x030007_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x030007_g_DeliveryList do
		if missionScriptId == x030007_g_DeliveryScript then
