...
x022001_g_DeliveryScript = 006668
x022001_g_DeliveryList = {1010102}
x022001_g_LootItemscript = 006667
x022001_g_LootItemList = {1030105}
--**********************************
--事件列表
--**********************************
function x022001_UpdateEventList( sceneId, selfId,targetId )
	...
	for i, Deliveryidx in x022001_g_DeliveryList do
		CallScriptFunction( x022001_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
	for i, LootItemidx in x022001_g_LootItemList do
		CallScriptFunction( x022001_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
	end
...
function x022001_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x022001_g_DeliveryList do
		if eventId == x022001_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x022001_g_LootItemList do
		if eventId == x022001_g_LootItemscript then
...
function x022001_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x022001_g_DeliveryList do
		if missionScriptId == x022001_g_DeliveryScript then
	...
	for i, LootItemidx in x022001_g_LootItemList do
		if missionScriptId == x022001_g_LootItemscript then
