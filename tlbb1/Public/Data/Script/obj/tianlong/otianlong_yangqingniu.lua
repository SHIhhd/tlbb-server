...
function x013014_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"#{QXQS_130106_09}")
		-- if	GetMenPai( sceneId, selfId) == 6 then
			-- if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 448))	then
				-- AddNumText(sceneId,x013014_g_ScriptId,"学习骑术：黄骠马",12,40)
			-- end
			-- if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 457))	then
				-- AddNumText(sceneId,x013014_g_ScriptId,"学习骑术：青白骢马",12,60)
			-- end
		-- end
		-- AddNumText( sceneId, x013014_g_ScriptId, "骑术介绍", 11, 100 )
	EndEvent(sceneId)
...
function x013014_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	-- if GetNumText() == 40 then
		-- BeginEvent( sceneId )
			...
			-- AddNumText(sceneId, x013014_g_ScriptId,"是",-1,0)
      -- AddNumText(sceneId, x013014_g_ScriptId,"否",-1,999)			
		-- EndEvent( sceneId )
		-- DispatchEventList( sceneId, selfId, targetId )
		-- return
	-- end
	...
	-- if GetNumText() == 60 then
		-- BeginEvent( sceneId )
			...
			-- AddNumText(sceneId, x013014_g_ScriptId,"是",-1,1)
      -- AddNumText(sceneId, x013014_g_ScriptId,"否",-1,999)			
		-- EndEvent( sceneId )
		-- DispatchEventList( sceneId, selfId, targetId )
		-- return
