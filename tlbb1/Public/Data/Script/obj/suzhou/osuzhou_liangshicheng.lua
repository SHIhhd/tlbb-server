...
--所拥有的事件ID列表
x001067_g_eventList={
	--889058, 889061六合和h1n1
	--魔尊魂武铸重楼
	--503030
}
x001067_g_DeliveryScript = 006668
x001067_g_DeliveryList = {1018717}
x001067_g_LootItemscript = 006667
x001067_g_LootItemList = {1039800}
--解决流失率兑换门派套装事件脚本
...
{n=9,id=20310101,num=1,str="重楼之泪"},
{n=10,id=20310102,num=1,str="重楼之芒"},
-- 新增特殊兔耳朵
{n=11,id=38002809,num=100,str="兔兔的书签"},
...
function x001067_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, Deliveryidx in x001067_g_DeliveryList do
			CallScriptFunction( x001067_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
		for i, LootItemidx in x001067_g_LootItemList do
			CallScriptFunction( x001067_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
		end
	EndEvent(sceneId)
...
function x001067_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	--配表任务
	for i,Deliveryidx in x001067_g_DeliveryList do
		if eventId == x001067_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x001067_g_LootItemList do
		if eventId == x001067_g_LootItemscript then
...
function x001067_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x001067_g_DeliveryList do
		if missionScriptId == x001067_g_DeliveryScript then
	...
	for i, LootItemidx in x001067_g_LootItemList do
		if missionScriptId == x001067_g_LootItemscript then
	...
	if x001067_g_EquipList[nItemIndex].n == 9100 then
		if selectRadioId == 10422016 then
			...
			if selectRadioId == 10423024 then
				...
				if selectRadioId == 10410035 then
					nLevel = 11
				end
			end
		end
