...
x001081_g_mpInfo		= {}
x001081_g_mpInfo[0]	= { "星宿", 16,  96, 152, MP_XINGSU }
x001081_g_mpInfo[1]	= { "逍遥", 14,  67, 145, MP_XIAOYAO }
x001081_g_mpInfo[2]	= { "少林",  9,  96, 127, MP_SHAOLIN }
x001081_g_mpInfo[3]	= { "天山", 17,  95, 120, MP_TIANSHAN }
x001081_g_mpInfo[4]	= { "天龙", 13,  96, 120, MP_DALI }
x001081_g_mpInfo[5]	= { "峨嵋", 15,  89, 139, MP_EMEI }
x001081_g_mpInfo[6]	= { "武当", 12, 103, 140, MP_WUDANG }
x001081_g_mpInfo[7]	= { "明教", 11,  98, 167, MP_MINGJIAO }
x001081_g_mpInfo[8]	= { "丐帮", 10,  91, 116, MP_GAIBANG }
x001081_g_mpInfo[10] = { "曼陀", SCENE_MANTUOSHANZHUANG,  139, 162, MP_MANTUO }
x001081_g_mpInfo[11] = { "恶人谷", 504,  122, 151, MP_ERENGU }
...
function x001081_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if GetItemCount(sceneId, selfId, x001081_g_Yinpiao)>=1  then
		...
		-- 我怎样才能去敦煌和嵩山
		AddNumText( sceneId, x001081_g_ScriptId, "我怎样才能去敦煌和嵩山", 11, 2000 )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x001081_g_ScriptId, "返回门派", 9, 1000 )
		AddNumText( sceneId, x001081_g_ScriptId, "城市 - 洛阳", 9, 1001 )
		AddNumText( sceneId, x001081_g_ScriptId, "城市 - 大理", 9, 1002 )
		AddNumText( sceneId, x001081_g_ScriptId, "城市 - 洛阳 - 九州商会", 9, 1006 )
    AddNumText( sceneId, x001081_g_ScriptId, "城市 - 楼兰", 9, 1007 )
    AddNumText( sceneId, x001081_g_ScriptId, "城市 - 束河古镇", 9, 1010 )
    
		AddNumText( sceneId, x001081_g_ScriptId, "带我去其它门派", 9, 1012 )
...
function x001081_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x001081_MsgBox( sceneId, selfId, targetId, "  你队伍成员中有人有漕运货舱在身，我们驿站不能为你提供传送服务。" )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x001081_MsgBox( sceneId, selfId, targetId, "  你有漕运货舱在身，我们驿站不能为你提供传送服务。" )
		return
	end
	...
	if arg == 1000 then		--返回门派
		if id == MP_WUMENPAI then
			x001081_MsgBox( sceneId, selfId, targetId, "  你还没有加入任何门派！" )
		else
