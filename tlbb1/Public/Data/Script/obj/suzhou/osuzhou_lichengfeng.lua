...
x001028_g_mpInfo[10] = { "曼陀", SCENE_MANTUOSHANZHUANG,  139, 162, MP_MANTUO }
x001028_g_mpInfo[11] = { "恶人谷", 504,  122, 151, MP_ERENGU }
...
function x001028_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x001028_MsgBox( sceneId, selfId, targetId, "  你队伍成员中有人有漕运货舱在身，我们驿站不能为你提供传送服务。" )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x001028_MsgBox( sceneId, selfId, targetId, "  你有漕运货舱在身，我们驿站不能为你提供传送服务。" )
		return
	end
	...
	if arg == 1000 then		--返回门派
		if id == MP_WUMENPAI then
			x001028_MsgBox( sceneId, selfId, targetId, "  你还没有加入任何门派！" )
		else
