...
--		AddNumText( sceneId, x001031_g_ScriptId, "#{FZCX_90627_1}", 6, 12 )	--查询上次繁殖等级
...
--		BeginUICommand( sceneId )
--			UICommand_AddInt( sceneId, targetId )
--			UICommand_AddInt( sceneId, 1 )				--ui type 1为查询
--		EndUICommand( sceneId )
--		DispatchUICommand( sceneId, selfId, 1100 )	--调用珍兽界面
...
--查询上次繁殖等级
--**********************************
--function x001031_OnLastPreCreateLevel( sceneId, selfId, targetId, petHid, petLid )
--	if petHid == nil or petLid == nil then
--		x001031_MsgBox( sceneId, selfId, targetId, "#{FZCX_90627_2}" )
--		return
--	end
--	--20090907 
--	--int1 0--just baby 1--ok
--	--int2 nLevel
...
--		EndUICommand( sceneId )
--		DispatchUICommand( sceneId, selfId, 20090907 )	--调用珍兽界面
--	end
--	local nLevel = LuaFnGetPetLastProcreateLevel(sceneId, selfId, petHid, petLid);
--	if nLevel < 0 then
--		nLevel = 0
--	end
--	BeginUICommand( sceneId )
--		UICommand_AddInt( sceneId, 1 )
--		UICommand_AddInt( sceneId, nLevel )
--	EndUICommand( sceneId )
--	DispatchUICommand( sceneId, selfId, 20090907 )	--调用珍兽界面
--end
--**********************************
...
function x001031_OnAcceptPetTitle( sceneId, selfId, targetId, petHid, petLid )
  ...
  if nSavvy ==10  then
	...
	--确定珍兽称号ID
	local	nTitle	= 100000 + 100 * nLevS + 10 * nLevG + nLevP + nGenLv
