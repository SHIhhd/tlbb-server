...
function x713550_OnDefaultEvent( sceneId, selfId, targetId, zhiwuId )
	...
	local nZhiWuLevel = V_ZHONGZHI_NEEDLEVEL[level_Index]
	--【25Q2神工值改造】
--	local EnergyCost = CallScriptFunction( ABILITYLOGIC_ID, "CalcEnergyCostCaiJi", sceneId, selfId, ABILITY_ZHONGZHI, nZhiWuLevel )
...
--		return
--	end
	local IBPowerCost = CallScriptFunction( ABILITYLOGIC_ID, "CalcIBPowerCostCaiJi", sceneId, selfId, ABILITY_ZHONGZHI, nZhiWuLevel )
	if LuaFnGetIBPower(sceneId,selfId) < IBPowerCost then
		BeginEvent(sceneId)
			AddText(sceneId, "#{SFJN_231225_7}")
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		return
	end
	...
	--增加熟练度
	CallScriptFunction(ABILITYLOGIC_ID, "GainExperience", sceneId, selfId, ABILITY_ZHONGZHI, nZhiWuLevel)
	--删除相应活力
--	CallScriptFunction(ABILITYLOGIC_ID, "EnergyCostZhongZhi", sceneId, selfId, ABILITY_ZHONGZHI, nZhiWuLevel)
	--【25Q2神工值改造】
	CallScriptFunction(ABILITYLOGIC_ID, "IBPowerCostCaiJi", sceneId, selfId, ABILITY_ZHONGZHI, nZhiWuLevel)
