...
function x001157_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1 then
		BeginEvent(sceneId)						
			...
			AddNumText(sceneId, x001157_g_scriptId, "取消", 8, 21 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	if GetNumText() == 14 then
		BeginEvent( sceneId )
			AddText( sceneId, "#{WPDJ_140912_1}" )
			AddNumText( sceneId, x001157_g_scriptId, "#{WPDJ_140912_2}", 6, 20 )
			AddNumText( sceneId, x001157_g_scriptId, "#{WPDJ_140912_3}", 0, 21 )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	end
	--开始合成
	if GetNumText() == 20 then
		--两种情况，一种是绑定情况，一种是非绑定情况，进入之前，先判定是否是绑定的。
		local nBindMatNum = LuaFnGetBindItemCountInBag(sceneId, selfId, 20310110) --优先寻找绑定材料
		if nBindMatNum > 0 then
			BeginEvent( sceneId )
				AddText( sceneId, "#{WPDJ_140912_8}" )
				AddNumText( sceneId, x001157_g_scriptId, "#{YDSS_15018_94}", 6, 22 )
				AddNumText( sceneId, x001157_g_scriptId, "#{YDSS_15018_95}", 0, 21 )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			if LuaFnGetAvailableItemCount(sceneId,selfId,20310110) < 5 then
				x001157_NotifyFailTips( sceneId, selfId,
					ScriptGlobal_Format("#{WPDJ_140912_4}"
					,GetItemName(sceneId,20310110)
					,GetItemName(sceneId,20310110))
				)
				return
			end
			if GetMoney(sceneId,selfId) + GetMoneyJZ(sceneId,selfId) < 10000 then
				x001157_NotifyFailTips(sceneId,selfId,"#{WPDJ_140912_5}")
				return
			end
			if LuaFnGetMaterialBagSpace(sceneId,selfId) < 1 then
				x001157_NotifyFailTips(sceneId,selfId,"#{WPDJ_140912_6}")
				return
			end
			--扣除金币，材料，给新材料
			LuaFnCostMoneyWithPriority(sceneId,selfId,10000);
			LuaFnDelAvailableItem(sceneId,selfId,20310110,5)
			--发送新物品
			LuaFnTryRecieveSingleItem(sceneId,selfId,20310111,1,0)
			x001157_NotifyFailTips(sceneId,selfId,
				ScriptGlobal_Format("#{WPDJ_140912_7}"
				,GetItemName(sceneId,20310111))
			)
	...
	if GetNumText() == 22 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,20310110,5)
		--发送新物品
		LuaFnTryRecieveSingleItem(sceneId,selfId,20310111,1,1)
		x001157_NotifyFailTips(sceneId,selfId,"#{WPDJ_140912_11}")
		BeginUICommand( sceneId )
		UICommand_AddInt( sceneId, targetId )
		EndUICommand( sceneId )
		DispatchUICommand( sceneId, selfId, 1000 )
