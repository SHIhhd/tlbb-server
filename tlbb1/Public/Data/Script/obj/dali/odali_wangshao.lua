...
--所拥有的事件ID列表
x002000_g_eventList = { 212117, 228902, 228904, 212140, 212144,891274 }
x002000_g_DeliveryScript = 006668
x002000_g_DeliveryList = {1010241,1010251,1010400}
x002000_g_LootItemscript = 006667
x002000_g_LootItemList = {1030220,1030221,1030222}
--**********************************
--事件列表
--**********************************
function x002000_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		for i, Deliveryidx in x002000_g_DeliveryList do
			CallScriptFunction( x002000_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
		for i, LootItemidx in x002000_g_LootItemList do
			CallScriptFunction( x002000_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
		end
...
function x002000_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x002000_g_DeliveryList do
		if eventId == x002000_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x002000_g_LootItemList do
		if eventId == x002000_g_LootItemscript then
...
function x002000_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x002000_g_DeliveryList do
		if missionScriptId == x002000_g_DeliveryScript then
	...
	for i, LootItemidx in x002000_g_LootItemList do
		if missionScriptId == x002000_g_LootItemscript then
