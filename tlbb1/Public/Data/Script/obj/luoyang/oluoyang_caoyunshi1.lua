...
x311006_g_DeliveryScript = 006668
x311006_g_DeliveryList = {1018721,1018832}
--龟兔赛跑任务号
...
function x311006_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		if x311006_OnIsFillPlayCard(sceneId, selfId, targetId) == 1 then
			...
			for i,Deliveryidx in x311006_g_DeliveryList do
				CallScriptFunction( x311006_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
			end
...
function x311006_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	--配表任务
	for i,Deliveryidx in x311006_g_DeliveryList do
		if eventId == x311006_g_DeliveryScript then
...
function x311006_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x311006_g_DeliveryList do
		if missionScriptId == x311006_g_DeliveryScript then
