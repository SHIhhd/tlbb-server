...
x000007_g_DeliveryScript = 006668
x000007_g_DeliveryList = 
{
	1010350,1010351,1010352,1010353,1010354,1010355,1010356,1010357,1010358,1010359,1010360,1010361,1010362,1010363,1010364,1010365,
	1010366,1010367,1010368,1010369,1010370,1010371,1010372,1010373,1010374,1010375,1010376,1010377,1010378,1010379,1010380,1010381,
	1010382,1010383,1010384,1010385,1010386,1010387,1010388,1010389
}
--**********************************
--事件列表
--**********************************
function x000007_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i,Deliveryidx in x000007_g_DeliveryList do
			CallScriptFunction( x000007_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
...
function x000007_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	--配表任务
	for i,Deliveryidx in x000007_g_DeliveryList do
		if eventId == x000007_g_DeliveryScript then
...
function x000007_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x000007_g_DeliveryList do
		if missionScriptId == x000007_g_DeliveryScript then
