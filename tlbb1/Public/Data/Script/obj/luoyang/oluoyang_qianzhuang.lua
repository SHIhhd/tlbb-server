...
--宠物银行允许扩容的等级
x000076_g_PetBankCanUseLev = 15
--兽栏契据
x000076_g_PetBankExternItem = 30606025
--宠物银行容器的大小
x000076_g_PetBankMaxCount = 10
...
function x000076_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		--宠物银行
		AddNumText(sceneId, 103, "#{ZSYH_120503_01}",5,-1)
		--宠物银行扩容
		local nCurEndIndex = GetPetBankRentIndex(sceneId,selfId)
		if nCurEndIndex < x000076_g_PetBankMaxCount then
			AddNumText(sceneId, 104, "#{ZSYH_120503_02}",5,-1)
		end
		AddNumText( sceneId, x000076_g_scriptId, "#{ZSYH_120503_03}", 11, 2)
		--关于保险箱
...
function x000076_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif eventId == 103 then
		...
		--打开珍兽银行
		if LuaFnGetLevel(sceneId,selfId) < x000076_g_PetBankCanUseLev then
			x000076_MsgBox( sceneId, selfId, "#{ZSYH_120503_34}" )
			x000076_MsgBoxTar(sceneId,selfId,targetId, "#{ZSYH_120503_33}")
			return
		end
		PetBankBegin(sceneId, selfId, targetId)	
		return
	elseif eventId == 104 then
		--扩充珍兽银行栏位
		x000076_AskExtendPetBank( sceneId, selfId, targetId )
		return
	elseif GetNumText() == 2 then
		--关于珍兽银行
		BeginEvent( sceneId )
			AddText( sceneId, "#{ZSYH_120503_04}" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	...
	elseif GetNumText() == 8 then
		x000076_RequestExtendPetBank( sceneId, selfId, targetId )
		return
	elseif GetNumText() == 9 then
		x000076_OnDefaultEvent( sceneId, selfId,targetId )
		return
	else
		--得到当前银行的存储格数
		local CurrentRentIndex = GetBankRentIndex(sceneId, selfId)
...
--宠物银行扩容执行
function x000076_RequestExtendPetBank( sceneId, selfId, targetId )
	if LuaFnGetAvailableItemCount(sceneId, selfId,x000076_g_PetBankExternItem) < 1 then
		x000076_MsgBox( sceneId, selfId, "#{ZSYH_120503_12}" )
		x000076_MsgBoxTar(sceneId,selfId,targetId, "#{ZSYH_120503_11}")
		return
	end
	local nCurEndIndex = GetPetBankRentIndex(sceneId,selfId)
	if nCurEndIndex >= x000076_g_PetBankMaxCount then
		x000076_MsgBox( sceneId, selfId, "#{ZSYH_120503_06}" )
		x000076_MsgBoxTar(sceneId,selfId,targetId, "#{ZSYH_120503_05}")
		return
	end
	--材料扣除
	LuaFnDelAvailableItem(sceneId, selfId,x000076_g_PetBankExternItem,1)
	local nNewEndIndex = nCurEndIndex + 1;
	SetPetBankRentIndex(sceneId,selfId,nNewEndIndex);
	if nNewEndIndex == x000076_g_PetBankMaxCount then
		x000076_MsgBox( sceneId, selfId, "#{ZSYH_120503_14}" )
		x000076_MsgBoxTar(sceneId,selfId,targetId, "#{ZSYH_120503_13}")
	else
		x000076_MsgBox( sceneId, selfId, 
			ScriptGlobal_Format("#{ZSYH_120503_16}",tostring(nNewEndIndex)) )
		x000076_MsgBoxTar(sceneId,selfId,targetId, 
			ScriptGlobal_Format("#{ZSYH_120503_15}",tostring(nNewEndIndex),tostring(x000076_g_PetBankMaxCount - nNewEndIndex)))
	end
	
end
--宠物银行扩容
function x000076_AskExtendPetBank( sceneId, selfId, targetId )
	if LuaFnGetLevel(sceneId,selfId) < x000076_g_PetBankCanUseLev then
		x000076_MsgBox( sceneId, selfId, "#{ZSYH_120503_08}" )
		x000076_MsgBoxTar(sceneId,selfId,targetId, "#{ZSYH_120503_07}")
		return
	end
	local nCurEndIndex = GetPetBankRentIndex(sceneId,selfId)
	if nCurEndIndex >= x000076_g_PetBankMaxCount then
	...
	if LuaFnIsPasswordSetup( sceneId, selfId, 0 ) == 1 then
		if LuaFnIsPasswordUnlocked( sceneId, selfId, 1 ) == 0 then
			x000076_MsgBoxTar(sceneId,selfId,targetId, "#{ZSYH_120503_55}")
			return
		end
	end
	BeginEvent(sceneId)
		AddText( sceneId, "#{ZSYH_120503_10}" )
		AddNumText( sceneId, x000076_g_scriptId, "确定", 0, 8)
		AddNumText( sceneId, x000076_g_scriptId, "返回", 0, 9)
	EndEvent(sceneId)
	DispatchEventList(sceneId,selfId,targetId)
end
...
--自定义函数，屏幕消息提示（只能显示一行，不能用#r换行，如需显示多行需分次调用。）
function x000076_MsgBoxTar( sceneId, selfId,targetId, msg )
	BeginEvent( sceneId )
		AddText( sceneId, msg )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
