...
x311007_g_DeliveryScript = 006668
x311007_g_DeliveryList = {1018723}
--龟兔赛跑任务号
...
function x311007_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		if x311007_OnIsFillPlayCard(sceneId, selfId, targetId) == 1 then
			...
			AddNumText( sceneId, x311007_g_scriptId, "了解漕运", 11, 1 )
			AddNumText( sceneId, x311007_g_scriptId, "开始漕运", 7, 2 )
			for i,Deliveryidx in x311007_g_DeliveryList do
				CallScriptFunction( x311007_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
			end
...
function x311007_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	--配表任务
	for i,Deliveryidx in x311007_g_DeliveryList do
		if eventId == x311007_g_DeliveryScript then
...
function x311007_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x311007_g_DeliveryList do
		if missionScriptId == x311007_g_DeliveryScript then
