...
function  x012007_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"#{QXQS_130106_06}")
		-- if	GetMenPai( sceneId, selfId) == 3 then
			-- if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 450))	then
				-- AddNumText(sceneId, x012007_g_ScriptId,"学习骑术：鹤",12,40)
			-- end
			-- if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 459))	then
				-- AddNumText(sceneId, x012007_g_ScriptId,"学习骑术：金翼鹤",12,60)
			-- end
		-- end
		-- AddNumText( sceneId, x012007_g_ScriptId, "骑术介绍", 11, 100 )
	EndEvent(sceneId)
...
function  x012007_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	-- if GetNumText() == 40 then
		-- BeginEvent( sceneId )
			...
			-- AddNumText(sceneId, x012007_g_ScriptId,"是",-1,0)
      -- AddNumText(sceneId, x012007_g_ScriptId,"否",-1,999)			
		-- EndEvent( sceneId )
		-- DispatchEventList( sceneId, selfId, targetId )
		-- return
	-- end
	...
	-- if GetNumText() == 60 then
		-- BeginEvent( sceneId )
			...
			-- AddNumText(sceneId, x012007_g_ScriptId,"是",-1,1)
      -- AddNumText(sceneId, x012007_g_ScriptId,"否",-1,999)			
		-- EndEvent( sceneId )
		-- DispatchEventList( sceneId, selfId, targetId )
		-- return
