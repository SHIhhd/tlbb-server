...
x012009_g_mpInfo[10] = { "曼陀", SCENE_MANTUOSHANZHUANG,  139, 162, MP_MANTUO }
x012009_g_mpInfo[11] = { "恶人谷", 504,  122, 151, MP_ERENGU }

--**********************************
--事件列表
--**********************************
function x012009_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, eventId in x012009_g_eventList do
...
function x012009_OnDefaultEvent( sceneId, selfId,targetId )
	x012009_UpdateEventList( sceneId, selfId, targetId )
end
...
function x012009_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	for i, findId in x012009_g_eventList do
		if eventId == findId then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId, GetNumText(),x012009_g_ScriptId )
			return
		end
	end
    ...
    if  GetNumText() == 11 then
		...
		BeginEvent(sceneId)
		    for i, mp in x012009_g_mpInfo do
				AddNumText( sceneId, x012009_g_ScriptId, "门派 - "..mp[1], 9, i+12 )
			end			
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)	
		return 
...
--接受此NPC的任务
--**********************************
function x012009_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x012009_g_eventList do
...
--拒绝此NPC的任务
--**********************************
function x012009_OnMissionRefuse( sceneId, selfId, targetId, missionScriptId )
	--拒绝之后，要返回NPC的事件列表
	for i, findId in x012009_g_eventList do
		if missionScriptId == findId then
			x012009_UpdateEventList( sceneId, selfId, targetId )
...
--继续（已经接了任务）
--**********************************
function x012009_OnMissionContinue( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x012009_g_eventList do
		if missionScriptId == findId then
...
--提交已做完的任务
--**********************************
function x012009_OnMissionSubmit( sceneId, selfId, targetId, missionScriptId, selectRadioId )
	for i, findId in x012009_g_eventList do
		if missionScriptId == findId then
...
--死亡事件
--**********************************
function x012009_OnDie( sceneId, selfId, killerId )
end
