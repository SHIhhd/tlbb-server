...
--所拥有的事件ID列表
x012014_g_eventList={229002,223000,223001,223002,223003,223004,223005,223006,223007,223008,223009,223010,223011,223012,223020,223900,050061,891279}
x012014_g_DeliveryScript = 006668
x012014_g_DeliveryList = {1010223,1018759,1018760,1018761,1018762,1018763,1018764,1018765,1018766,1018767,1018768}
x012014_g_LootItemscript = 006667
x012014_g_LootItemList = {1039807}
--**********************************
--事件交互入口
--**********************************
function x012014_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, Deliveryidx in x012014_g_DeliveryList do
			CallScriptFunction( x012014_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
		for i, LootItemidx in x012014_g_LootItemList do
			CallScriptFunction( x012014_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
		end
		...
		CallScriptFunction( 229013, "OnEnumerate",sceneId, selfId, targetId, MP_WUDANG )
...
function x012014_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif eventId == 229013 then
		CallScriptFunction( 229013, "OnDefaultEvent",sceneId, selfId, targetId, MP_WUDANG, GetNumText() )
		return
	...
	--配表任务
	for i,Deliveryidx in x012014_g_DeliveryList do
		if eventId == x012014_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x012014_g_LootItemList do
		if eventId == x012014_g_LootItemscript then
...
function x012014_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x012014_g_DeliveryList do
		if missionScriptId == x012014_g_DeliveryScript then
	...
	for i, LootItemidx in x012014_g_LootItemList do
		if missionScriptId == x012014_g_LootItemscript then
