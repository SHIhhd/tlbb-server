...
x031004_g_DeliveryScript = 006668
x031004_g_DeliveryList = {1010141}
x031004_g_KillMonsterScript = 006666
x031004_g_KillMonsterList = {1000141,1000142,1000143,1000352}
--**********************************
--事件列表
--**********************************
function x031004_UpdateEventList( sceneId, selfId,targetId )
	...
	for i, Deliveryidx in x031004_g_DeliveryList do
		CallScriptFunction( x031004_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
	for i,KillMonsteridx in x031004_g_KillMonsterList do
		CallScriptFunction( x031004_g_KillMonsterScript, "OnEnumerate",sceneId, selfId, targetId,KillMonsteridx)
	end
...
function x031004_OnEventRequest( sceneId, selfId, targetId, eventId )
	for i,Deliveryidx in x031004_g_DeliveryList do
		if eventId == x031004_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId,Deliveryidx)
		end
	end
	for i,KillMonsteridx in x031004_g_KillMonsterList do
		if eventId == x031004_g_KillMonsterScript then
...
function x031004_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	local ret = 0
	for i, Deliveryidx in x031004_g_DeliveryList do
		if missionScriptId == x031004_g_DeliveryScript then
	...
	for i,KillMonsteridx in x031004_g_KillMonsterList do
		if missionScriptId == x031004_g_KillMonsterScript then
