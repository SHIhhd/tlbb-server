...
x031005_g_LootItemscript = 006667
x031005_g_LootItemList = {1030140,1030141}
--**********************************
--事件列表
--**********************************
function x031005_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
	AddText(sceneId,"五月五日长生殿，夜半无人私语时。")
	for i, LootItemidx in x031005_g_LootItemList do
		CallScriptFunction( x031005_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
	end
...
function x031005_OnEventRequest( sceneId, selfId, targetId, eventId )
	for i, LootItemidx in x031005_g_LootItemList do
		if eventId == x031005_g_LootItemscript then
...
function x031005_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i, LootItemidx in x031005_g_LootItemList do
		if missionScriptId == x031005_g_LootItemscript then
