...
--所拥有的事件ID列表
x031009_g_eventList={}
x031009_g_DeliveryScript = 006668
x031009_g_DeliveryList = {1010142}
--**********************************
--事件列表
--**********************************
function x031009_UpdateEventList( sceneId, selfId,targetId )
	...
	for i, Deliveryidx in x031009_g_DeliveryList do
		CallScriptFunction( x031009_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
...
function x031009_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x031009_g_DeliveryList do
		if eventId == x031009_g_DeliveryScript then
...
function x031009_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x031009_g_DeliveryList do
		if missionScriptId == x031009_g_DeliveryScript then
