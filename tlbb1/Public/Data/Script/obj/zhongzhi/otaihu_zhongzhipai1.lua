...
--脚本号
x714211_g_ScriptId = 714211

event_xuanzezhiwu = 713550

--植物产品编号列表
x714211_g_eventList={20104004,20104005, 20104006, 20104010, 20105004,20105005,20105006} --植物的编号,不是scriptId
--**********************************
--事件交互入口
--**********************************
function x714211_OnDefaultEvent( sceneId, selfId,targetId )
	--PLANTFLAG[1] =0
	--PLANTFLAG[2] =0
	...
	if AbilityLevel >= 4	then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x714211_g_ScriptId, "种植早产植物",6,254)
			AddNumText(sceneId, x714211_g_ScriptId, "种植晚产植物",6,255)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	else
		BeginEvent(sceneId)
			AddText(sceneId, "你种植技能等级太低了，最低也得达到4级吧")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
--事件列表选中一项
--**********************************
function x714211_OnEventRequest( sceneId, selfId, targetId, eventId )
	local name_Index
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--通过x714211_g_eventList和scriptglobal中的植物列表对比，并根据玩家种植技能等级来显示相应植物
			for i, eventId in x714211_g_eventList do	--遍历这个稻草人可以种植的植物列表
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--遍历scriptglobal中的所有植物列表
					if eventId == g_ZhiWuId then
						...
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --如果玩家种植技能等级>=该植物要求技能等级]
							...
							AddNumText(sceneId, x714211_g_ScriptId, V_ZHONGZHI_NAME[name_Index].."("..V_ZHONGZHI_NEEDLEVEL[j].."级)",6,name_Index)
							break
						end
					end
				end
	...
	zhiwuId = NumText
	for i, findId in x714211_g_eventList do
...
--接受此NPC的任务（暂时借用任务接口）
--**********************************
function x714211_OnMissionSubmit( sceneId, selfId, targetId, scriptId )
	for i, findId in x714211_g_eventList do
...
--接受此NPC的任务
--**********************************
function x714211_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x714211_g_eventList do
