...
--		if CityGetSelfCityID( sceneId, selfId ) == sceneId then
			...
			--回到城市
			AddNumText( sceneId, x805009_g_scriptId, "#{BHCS_090226_10}", 13, 29 )
			...
			if GetGuildPos( sceneId, selfId ) == GUILD_POSITION_CHIEFTAIN then
				if CityGetBuildingLevel(sceneId,selfId,sceneId,CITY_BUILDING_XIANYA) < 4 then
					AddNumText( sceneId, x805009_g_scriptId, "#H升级县衙及厢房到5级", 6, 31 )
				end
			end
...
--			print("currGuildContriTitle:"..currGuildContriTitle)
...
function x805009_OnEventRequest( sceneId, selfId, targetId, eventId )
 ...
 --回到大理
	...
	elseif GetNumText() == 29 then
		...
		AddNumText( sceneId, x805009_g_scriptId, "#{BHCS_151106_12}", 9, 32 )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	...
	elseif GetNumText() == 31 then
		...
		if CityGetBuildingLevel(sceneId,selfId,sceneId,CITY_BUILDING_XIANYA) >= 4 then
			BeginEvent( sceneId )
				AddText( sceneId, "  现在的等级已经满了啊，请你不要来捣乱了！" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
		-- CitySetLevel(sceneId, selfId, sceneId, 4)
		CityBuildingChange(sceneId, selfId, CITY_BUILDING_XIANYA, 4)
		CityBuildingChange(sceneId, selfId, CITY_BUILDING_XIANGFANG, 4)
		
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_JIUSI, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_QIANZHUANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_FANGJUFANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_DUANTAI, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_WUJUFANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_MICANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_CHENGQIANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_JIFANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_YISHE, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_WUFANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_JIANTA, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_SHUFANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_QIJI, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_XIAOCHANG, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_HUOBINGTA, 4)
		-- CityBuildingChange(sceneId, selfId, CITY_BUILDING_DAQI, 4)
		
		BeginEvent( sceneId )
			AddText( sceneId, "  你的帮会城市及户部已升级到5级！" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	elseif GetNumText() == 32 then
		...
		CallScriptFunction((400900), "TransferFuncFromNpc",sceneId, selfId, SCENE_LOULAN,188,130)
	end
end
