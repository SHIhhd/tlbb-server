...
x010012_g_DeliveryScript = 006668
x010012_g_DeliveryList = {1010221,1018739,1018740,1018741,1018742,1018743,1018744,1018745,1018746,1018747,1018748}
x010012_g_LootItemscript = 006667
x010012_g_LootItemList = {1039806}
--**********************************
--事件交互入口
--**********************************
function x010012_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, Deliveryidx in x010012_g_DeliveryList do
			CallScriptFunction( x010012_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
		end
		for i, LootItemidx in x010012_g_LootItemList do
			CallScriptFunction( x010012_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
		end
		...
		CallScriptFunction( 229013, "OnEnumerate",sceneId, selfId, targetId, MP_GAIBANG )
...
function x010012_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif eventId == 229013 then
		CallScriptFunction( 229013, "OnDefaultEvent",sceneId, selfId, targetId, MP_GAIBANG, GetNumText() )
		return
	...
	--配表任务
	for i,Deliveryidx in x010012_g_DeliveryList do
		if eventId == x010012_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x010012_g_LootItemList do
		if eventId == x010012_g_LootItemscript then
...
function x010012_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x010012_g_DeliveryList do
		if missionScriptId == x010012_g_DeliveryScript then
	...
	for i, LootItemidx in x010012_g_LootItemList do
		if missionScriptId == x010012_g_LootItemscript then
