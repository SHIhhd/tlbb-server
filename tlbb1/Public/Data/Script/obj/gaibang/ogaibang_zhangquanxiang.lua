...
x010008_g_mpInfo[10] = { "曼陀", SCENE_MANTUOSHANZHUANG,  139, 162, MP_MANTUO }
x010008_g_mpInfo[11] = { "恶人谷", 504,  122, 151, MP_ERENGU }

--**********************************
--事件列表
--**********************************
function x010008_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, eventId in x010008_g_eventList do
...
function x010008_OnDefaultEvent( sceneId, selfId,targetId )
	x010008_UpdateEventList( sceneId, selfId, targetId )
end
...
function x010008_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	for i, findId in x010008_g_eventList do
		if eventId == findId then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId, GetNumText(),x010008_g_ScriptId )
			return
		end
	end
    ...
    if  GetNumText() == 11 then
		...
		BeginEvent(sceneId)
		    for i, mp in x010008_g_mpInfo do
				AddNumText( sceneId, x010008_g_ScriptId, "门派 - "..mp[1], 9, i+12 )
			end	
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)	
		return 
...
--接受此NPC的任务
--**********************************
function x010008_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x010008_g_eventList do
...
--拒绝此NPC的任务
--**********************************
function x010008_OnMissionRefuse( sceneId, selfId, targetId, missionScriptId )
	--拒绝之后，要返回NPC的事件列表
	for i, findId in x010008_g_eventList do
		if missionScriptId == findId then
			x010008_UpdateEventList( sceneId, selfId, targetId )
...
--继续（已经接了任务）
--**********************************
function x010008_OnMissionContinue( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x010008_g_eventList do
		if missionScriptId == findId then
...
--提交已做完的任务
--**********************************
function x010008_OnMissionSubmit( sceneId, selfId, targetId, missionScriptId, selectRadioId )
	for i, findId in x010008_g_eventList do
		if missionScriptId == findId then
...
--死亡事件
--**********************************
function x010008_OnDie( sceneId, selfId, killerId )
end
