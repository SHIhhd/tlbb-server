...
--脚本号
x340001_g_scriptId = 340001 --临时写这个,真正用的时候一定要改.

--需要的等级

--效果的ID
x340001_g_Impact1 = 340001 --临时写这个
x340001_g_Impact2 = -1 --不用

x340001_g_Impact_NotTransportList = { 5929 } -- 禁止传送的Impact
x340001_g_TalkInfo_NotTransportList = { "#{GodFire_Info_062}" } -- 禁止传送的Impact提示信息
...
function x340001_OnConditionCheck( sceneId, selfId )
	...
	if GetItemCount(sceneId, selfId, 40002000)>=1  then
		BeginEvent( sceneId )
			AddText( sceneId, "你身上有银票，正在跑商！不可以使用传送功能。" )
		EndEvent( sceneId )
		DispatchMissionTips( sceneId, selfId )
		return 0
	end
...
function x340001_MenpaiTransfer( sceneId, selfId )
    ...
    if( MenPai ~= MP_WUMENPAI ) then
        ...
        if MP_MANTUO == MenPai then      --曼陀山庄
            TargetScene = SCENE_MANTUOSHANZHUANG
            x = 152
            z = 113                  
        end
