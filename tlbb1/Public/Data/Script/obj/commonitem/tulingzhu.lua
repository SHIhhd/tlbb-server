...
x330001_g_ItemIdZhanBei = 30505288
--效果的ID
...
function x330001_OnConditionCheck( sceneId, selfId )
	...
	--具体物品的代码
	local bagItemIndex = LuaFnGetItemIndexOfUsedItem(sceneId,selfId)
	...
	--战备土灵珠特写
	if bagItemIndex == x330001_g_ItemIdZhanBei then
		otim	= GetBagItemParam( sceneId, selfId, bagId, 10, 0 )
	else
...
function x330001_SetPosition( sceneId, selfId, nItemIndex )
	...
	if GetItemTableIndexByIndex(sceneId, selfId, nItemIndex) ~= x330001_g_ItemId 
		and GetItemTableIndexByIndex(sceneId, selfId, nItemIndex) ~= x330001_g_ItemId01
		and GetItemTableIndexByIndex(sceneId, selfId, nItemIndex) ~= x330001_g_ItemIdZhanBei then
	...
	local	opx, opy	--记忆坐标
	--战备土灵珠特写
	if GetItemTableIndexByIndex(sceneId, selfId, nItemIndex) == x330001_g_ItemIdZhanBei then
		otim	= GetBagItemParam( sceneId, selfId, nItemIndex, 10, 0 )
	else
	...
	--战备土灵珠特写
	if GetItemTableIndexByIndex(sceneId, selfId, nItemIndex) == x330001_g_ItemIdZhanBei then
		SetBagItemParam( sceneId, selfId, nItemIndex, 10, 0, otim )								--剩余使用次数
	else
