...
--所拥有的事件ID列表
x024004_g_eventList={}
x024004_g_DeliveryScript = 006668
x024004_g_DeliveryList = {1010111}
x024004_g_LootItemscript = 006667
x024004_g_LootItemList = {1030111}
--**********************************
--事件列表
--**********************************
function x024004_UpdateEventList( sceneId, selfId,targetId )
	...
	for i, Deliveryidx in x024004_g_DeliveryList do
		CallScriptFunction( x024004_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
	for i, LootItemidx in x024004_g_LootItemList do
		CallScriptFunction( x024004_g_LootItemscript, "OnEnumerate",sceneId, selfId, targetId ,LootItemidx)
	end
...
function x024004_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x024004_g_DeliveryList do
		if eventId == x024004_g_DeliveryScript then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId ,Deliveryidx)
		end
	end
	for i, LootItemidx in x024004_g_LootItemList do
		if eventId == x024004_g_LootItemscript then
...
function x024004_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x024004_g_DeliveryList do
		if missionScriptId == x024004_g_DeliveryScript then
	...
	for i, LootItemidx in x024004_g_LootItemList do
		if missionScriptId == x024004_g_LootItemscript then
