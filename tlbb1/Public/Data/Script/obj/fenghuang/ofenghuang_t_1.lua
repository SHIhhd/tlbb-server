x403050_g_scriptId = 403050
x403050_g_PosData = 
{
	{posx = 251,posz = 27},{posx = 289,posz = 64},{posx = 292,posz = 252},{posx = 258,posz = 291},
	{posx = 66,posz = 292},{posx = 28,posz = 262},{posx = 35,posz = 60},{posx = 150,posz = 146}
}
--**********************************
--事件交互入口
--**********************************
function x403050_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"#{FHZD_090708_48}")
		if sceneId == 180 then
			AddNumText(sceneId, x403051_g_scriptId,"#{FHZD_XML_20}",9,1)
...
--事件列表选中一项
--**********************************
function x403050_OnEventRequest( sceneId, selfId, targetId, eventId )
		...
		if nWeek == 3 or nWeek == 6 then
			if nTime >= 2030 or nTime < 2200 then
				BeginEvent(sceneId)
					...
					AddNumText(sceneId, x403050_g_scriptId,"确定",9,nNumText + 1000)
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,targetId)
				return
			end
		else
			CallScriptFunction((400900), "TransferFunc",sceneId,selfId,SCENE_SHUHE,x403050_g_PosData[8].posx,x403050_g_PosData[8].posz)
			return
		end
	else
		if sceneId == SCENE_PHOENIXPLAIN then
			SetPos(sceneId,selfId,x403050_g_PosData[nNumText].posx,x403050_g_PosData[nNumText].posz)
		end
	end
