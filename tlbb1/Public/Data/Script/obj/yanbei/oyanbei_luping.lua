...
x019000_g_DeliveryScript = 006668
x019000_g_DeliveryList = {1010062}
--**********************************
--事件列表
--**********************************
function x019000_UpdateEventList( sceneId, selfId,targetId )
	...
	for i, Deliveryidx in x019000_g_DeliveryList do
		CallScriptFunction( x019000_g_DeliveryScript, "OnEnumerate",sceneId, selfId, targetId ,Deliveryidx)
	end
...
function x019000_OnEventRequest( sceneId, selfId, targetId, eventId )
	--配表任务
	for i,Deliveryidx in x019000_g_DeliveryList do
		if eventId == x019000_g_DeliveryScript then
...
function x019000_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	...
	local ret = 0
	for i,Deliveryidx in x019000_g_DeliveryList do
		if missionScriptId == x019000_g_DeliveryScript then
