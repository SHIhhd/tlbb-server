...
x050220_g_CopySceneMap = "xuanmuchang.path"
...
x050220_g_TakeTimes = 99											-- 每天最多领取次数
--**********************************
-- 任务入口函数
--**********************************
...
function x050220_CheckAccept( sceneId, selfId, targetId )
	...
	--5次
	local takenTimes = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_HUANGJINZHILIAN)
...
function x050220_OnAccept( sceneId, selfId, targetId )
	...
	-- 一天只能领 x050220_g_TakeTimes 次
	AddCampaignCountNum(sceneId,selfId,CAMPAIGN_HUANGJINZHILIAN)
...
function x050220_OnPlayerEnter( sceneId, selfId )
	--设置死亡后复活点位置
	SetPlayerDefaultReliveInfo(sceneId,selfId,"%50","%50","%10",sceneId, x050220_g_Fuben_X,x050220_g_Fuben_Z);
	...
	elseif szName == x050220_g_BossWang then-- boss王阎
		...
		for i = 0, num - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCharacterLiving( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,mems[i] ) == 1 then						-- 不在场景的不做此操作
				...
				CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, mems[i],CAMPAIGN_HUANGJINZHILIAN)
			end
		end
			...
			-- if NotifyTime == 0 then --首次提醒
				-- for	i = 0, membercount - 1 do
					-- if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then
						-- local Minute = 0
						-- if 60 - TickCount < 12 then--小于1分钟按1分钟提示
							-- Minute = 1
						-- else
...
-- 获取本副本的MonsterID
--**********************************
function x050220_GetFubenMonsterDataId(sceneId,selfId,nIndex)
	local BossWangID = BossWangIDTbl[1]
	if nIndex == 1 then --王阎
