...
{
	MapFile = "loulanxunbao.path", AreaFile = "loulanxunbao_area.ini", MonsterFile = "loulanxunbao_monster.ini",	
}
	...
	elseif killednumber > x808039_g_TotalNeedKill then --龙王都被干掉了
		...
		for i=0,num-1 do
			...
			if LuaFnIsObjValid(sceneId, humanObjId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, humanObjId) == 1 then
				...
				DispatchMissionTips(sceneId,humanObjId)
				--周活跃数据埋点
				LuaFnFinishZhouHuoYueMission(sceneId,humanObjId,21)
				--英雄大会埋点
				if LuaFnHaveImpactOfSpecificDataIndex(sceneId,humanObjId,10541) == 1 then
					AddHerosReturnsRecoed(sceneId,humanObjId,13)
				end
			end
		end
		-- boss 击杀公告
...
function x808039_CheckMemberInfo(sceneId, selfId, targetId)
  ...
  for i=0, teamSize-1 do
  	...
  	local bXinfaOK = ScriptGlobal_CheckAllXinfaLevel(sceneId, objId, 40) --心法达到40级？
