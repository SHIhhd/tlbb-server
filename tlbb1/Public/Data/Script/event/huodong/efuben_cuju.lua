...
x402040_g_CopySceneMap = "cuju.path"
...
function x402040_OnDefaultEvent( sceneId, selfId, targetId )
	...
	for	i=0, nearteammembercount-1    do
		local memId = GetNearTeamMember(sceneId, selfId, i)
		if ScriptGlobal_CheckAllXinfaLevel(sceneId, memId, 30) == 0  then
			BeginEvent(sceneId)
				AddText(sceneId,"#B横扫牡丹腕");
...
function x402040_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "cuju.path"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
