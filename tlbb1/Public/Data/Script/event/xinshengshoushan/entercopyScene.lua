		...
		elseif (nCheckRet == 5 or nCheckRet == 6) then
			...
			for i = 0, nearMemberCount - 1 do
				...
				if sceneMemId and sceneMemId >= 0 then			
					...
					if (0 == ScriptGlobal_CheckAllXinfaLevel(sceneId, sceneMemId,x808121_g_Low_Level_xinfa)) then
...
function x808121_CheckCanEnterScene(sceneId, selfId, targetId, eventId)
	...
	-- 检测队长的等级是否满足要求
	if (GetLevel(sceneId, selfId) < x808121_g_Low_Level or 0 == ScriptGlobal_CheckAllXinfaLevel(sceneId, selfId,x808121_g_Low_Level_xinfa)) then
		return 5
	end
