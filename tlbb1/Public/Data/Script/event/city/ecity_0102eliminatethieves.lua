...
--任务文本描述
x600004_g_MissionName = "工程任务"
x600004_g_MissionInfo = ""													--任务描述
x600004_g_MissionTarget = "    消灭所有捣乱的小贼。"						--任务目标
x600004_g_ContinueInfo = "    你的任务还没有完成么？"						--未完成任务的npc对话
x600004_g_SubmitInfo = "    事情进展得如何？"								--完成未提交时的npc对话
x600004_g_MissionComplete = "    干得不错，甚好甚好。"						--完成任务npc说话的话
...
--副本名称
x600004_g_CopySceneName = "矿场"

x600004_g_CopySceneType = FUBEN_MINE	--副本类型，定义在ScriptGlobal.lua里面

x600004_g_CopySceneMap = "kuangchang.path"
...
function x600004_AskEnterCopyScene( sceneId, selfId )
	...
	local missionInfo
	if random(2) == 1 then
		missionInfo = format( "    城中矿场，有一群小贼前来捣乱，需要你去赶走他们。" )
	else
		missionInfo = format( "    有一群小贼，无端在我们矿场闹事，就看你的了。" )
	end
	...
	AddText( sceneId, "    杀死矿场里 " .. KillNumber .. " 个捣乱的小贼。" )
	AddNumText( sceneId, x600004_g_ScriptId, "进入矿场", 10, 1 )
end
...
function x600004_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x600004_g_MissionId ) == 0 then					--没有任务才可以走这里
		if GetLevel( sceneId, selfId ) < 20 then
			CallScriptFunction( x600004_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "等级不够" )
			return
		end
		...
		BeginEvent( sceneId )
			AddText( sceneId, x600004_g_MissionName )
			x600004_AskEnterCopyScene( sceneId, selfId )
			AddText( sceneId, "#r    你接受了任务：" .. x600004_g_MissionName )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	end
end
...
function x600004_AcceptEnterCopyScene( sceneId, selfId )
	if IsHaveMission( sceneId, selfId, x600004_g_MissionId ) > 0 then					--有任务才可以走这里
		...
		--add by xindefeng
		SetMissionByIndex( sceneId, selfId, misIndex, x600004_g_Param_killcount, 0 )	--将任务的第5号数据设置为0, 设置玩家杀死怪物数量为0
...
function x600004_MakeCopyScene( sceneId, selfId )
	...
	if bRetSceneID > 0 then
		x600004_NotifyFailTips( sceneId, selfId, "副本创建成功！" )
	else
		x600004_NotifyFailTips( sceneId, selfId, "副本数量已达上限，请稍候再试！" )
	end
...
function x600004_OnPlayerEnter( sceneId, selfId )
	if IsHaveMission( sceneId, selfId, x600004_g_MissionId ) == 0 then				--如果进入副本前删除任务，则直接传送回
		x600004_NotifyFailTips( sceneId, selfId, "你当前未接此任务" )
...
function x600004_OnAbandon( sceneId, selfId )
	...
	if sceneId == copyscene then											--如果在副本里删除任务，则直接传送回
		x600004_NotifyFailTips( sceneId, selfId, "任务失败！" )
		local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, 3 )		--取得副本入口场景号
		NewWorld( sceneId, selfId, oldsceneId, x600004_g_Back_X, x600004_g_Back_Z )
	end
end
...
function x600004_BackToCity( sceneId, selfId )
	--changed by xindefeng
	--副本新机制:见x600004_AcceptEnterCopyScene()
	...
	--本想修改x600004_g_CityMissionScript的x600004_BackToCity()函数,但怕引用它的脚本太多,所以本脚本不引用
	local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, 3 )					--取得进入副本时所在场景号
	CallScriptFunction( 400900, "TransferFunc", sceneId, selfId, oldsceneId, x600004_g_Back_X, x600004_g_Back_Z )	--传送
end
...
function x600004_OnKillObject( sceneId, selfId, objdataId, objId )
	...
	local strText = format( "已杀死捣乱的小贼：%d/%d", killedbossnumber, KillNumber )
	...
	if killedbossnumber >= KillNumber then
		...
		strText = format( "任务完成，将在 %d 秒后传送到入口位置", x600004_g_CloseTick * x600004_g_TickTime )
		x600004_NotifyFailTips( sceneId, selfId, strText )
...
function x600004_OnCopySceneTimer( sceneId, nowTime )
	...
	if leaveFlag == 1 then															--需要离开
		...
		if leaveTickCount >= x600004_g_CloseTick then										--倒计时间到，大家都出去吧
			...
			local strText = format( "你将在 %d 秒后离开场景", ( x600004_g_CloseTick - leaveTickCount ) * x600004_g_TickTime )
