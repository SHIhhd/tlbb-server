...
x600016_g_MissionTarget = "    从%n夺回%s个%i。"						--任务目标
...
function x600016_OnAccept( sceneId, selfId, targetId )
	...
	if GetLevel( sceneId, selfId ) < 40 then
		CallScriptFunction( x600016_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "等级不够" )
		return
	end
	...
	if not validMissions then
		...
		CallScriptFunction( x600016_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "无法找到合适的任务" )
		return
	end
	...
	BeginEvent( sceneId )
		local missionInfo
		if random(2) == 1 then
			missionInfo = format( "    本城在送货时，路过%s地方，遭遇那里的%s袭击，导致%d个#Y%s#W被抢，夺回之事非你不可啊。",
				strNpcSceneDesc, strNpcName, itemCount, strDemandItemName )
		else
			missionInfo = format( "    %s%s昨夜袭击了书院，丢失了%d个#Y%s#W，看你的了。",
				strNpcSceneDesc, strNpcName, itemCount, strDemandItemName )
		end

		AddText( sceneId, missionInfo )
		AddText( sceneId, "#r    你接受了任务：" .. x600016_g_MissionName )
	EndEvent( sceneId )
