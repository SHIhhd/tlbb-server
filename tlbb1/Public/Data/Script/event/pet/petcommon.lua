...
-- 生成两只不同性别的RMB珍兽
--**********************************
function x800105_CreateRMBDiffSexPetToHuman(sceneId, selfId, petDataId, level)
	if not sceneId or not selfId or not petDataId then
		return 0;
	end
	...
	local ret, petGUID_F_H, petGUID_F_L,petGUID_M_H,petGUID_M_L = LuaFnCreatePetDiffSexToHuman(sceneId, selfId, petDataId, level, 1);
	if not ret or ret ~= 1 or not petGUID_F_H or not petGUID_F_L or not petGUID_M_H or not petGUID_M_L then
		x800105_ShowTips(sceneId, selfId, "你不能携带此珍兽。");
		return 0;
	end
	return 1, petGUID_F_H, petGUID_F_L,petGUID_M_H,petGUID_M_L
end

--**********************************
-- 记录使用推荐加点的珍兽
--**********************************
function x800105_PetRecommendAuditLog(sceneId, selfId, hGUID, lGUID,nAttackTraitId)
	if not hGUID or not lGUID or not nAttackTraitId then
		return 0;
	end
	--print("PetRecommendAuditLog hGUID lGUID nAttackTraitId")
end
