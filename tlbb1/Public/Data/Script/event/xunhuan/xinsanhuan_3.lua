...
x050222_g_CopySceneMap = "huomuchang.path"
...
x050222_g_TakeTimes = 99											-- 每天最多领取次数
--**********************************
-- 任务入口函数
--**********************************
...
function x050222_CheckAccept( sceneId, selfId, targetId )
	...
	-- end
	local takenTimes = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_RONGYANZHIDI)
	--5次
...
function x050222_OnAccept( sceneId, selfId, targetId )
	...
	-- 一天只能领 x050222_g_TakeTimes 次
	AddCampaignCountNum(sceneId,selfId,CAMPAIGN_RONGYANZHIDI)
...
function x050222_OnPlayerEnter( sceneId, selfId )  
	--设置死亡后复活点位置
	SetPlayerDefaultReliveInfo(sceneId,selfId,"%50","%50","%10",sceneId, x050222_g_Fuben_X,x050222_g_Fuben_Z);
end
	...
	if szName == x050222_g_BossHuoYanYao then
		...
		for i = 0, num - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCharacterLiving( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,mems[i] ) == 1 then						-- 不在场景的不做此操作
				...
				--周活跃数据埋点
				LuaFnFinishZhouHuoYueMission(sceneId,mems[i],11)
				--英雄大会埋点
				if LuaFnHaveImpactOfSpecificDataIndex(sceneId,mems[i],10541) == 1 then
					AddHerosReturnsRecoed(sceneId,mems[i],12)
				end
				CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, mems[i],CAMPAIGN_RONGYANZHIDI)
			end
		end
...
-- 获取本副本的MonsterID
--**********************************
function x050222_GetFubenMonsterDataId(sceneId,selfId,nIndex)
	local BossHuoYanYaoID = BossHuoYanYaoIDTbl[1]
	if nIndex == 1 then --火焰妖魔
		...
		if BossHuoYanYaoIDTbl[iniLevel] then
			BossHuoYanYaoID = BossHuoYanYaoIDTbl[iniLevel]
		end
	end
	return BossHuoYanYaoID,1
end