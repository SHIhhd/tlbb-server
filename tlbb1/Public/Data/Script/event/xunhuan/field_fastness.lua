...
x050102_g_CopySceneMap = "yewai.path"
...
x050102_g_TakeTimes = 99											-- 每天最多领取次数
x050102_g_RelationReward = 10									-- 关系值奖励
x050102_g_MaxRelation = 9999									-- 关系最大值
x050102_g_Boss = {4130,4131,4132,4133,4134,4135,4136,4137,4138,4139,34130,34131,34132,34133,34134,34135,34136,34137,34138,34139}
--**********************************
-- 任务入口函数
--**********************************
...
function x050102_CheckAccept( sceneId, selfId )
	...
	-- if nowDate ~= oldDate then
		-- takenTimes = 0
	-- end
	local takenTimes = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_JIAOFEI)
...
function x050102_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050102_g_MissionId ) == 0 then					-- 没有任务才可以走这里
		...
		-- 一天只能领 x050102_g_TakeTimes 次
		AddCampaignCountNum(sceneId,selfId,CAMPAIGN_JIAOFEI)
		-- local DayTimes
	...
	if leaveFlag == 1 then
		...
		for i = 1, num do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 then						-- 不在场景的不做此操作
				...
				CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, mems[i],CAMPAIGN_JIAOFEI)
				LuaFnFinishZhouHuoYueMission(sceneId,mems[i],10)
				--英雄大会埋点
...
-- 获取本副本的MonsterID
--**********************************
function x050102_GetFubenMonsterDataId( sceneId, selfId,nIndex)
	--nIndex 1-6
	--余毒1个
	local mylevel = GetLevel(sceneId,selfId)
	local iniLevel = floor( mylevel/10 )
	if not x050102_g_Boss[iniLevel] then
		return x050102_g_Boss[getn(x050102_g_Boss)],1
	end
	return x050102_g_Boss[iniLevel],1
end
--**********************************
-- 对话窗口信息提示
--**********************************
