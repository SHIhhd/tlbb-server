...
--任务文本描述
x808016_g_MissionName			= "偷袭门派"
--任务描述
x808016_g_MissionInfo			= "  "
--任务目标
x808016_g_MissionTarget		= "  杀死所有的怪物即可完成任务。"
--未完成任务的npc对话
...
--**********************************以上是动态****************************
--角色Mission变量说明
x808016_g_Param_IsMissionOkFail	= 0						--0号：当前任务是否完成(0未完成；1完成)
x808016_g_Param_killmonstercount	= 1					--1号：杀死任务小怪的数量
x808016_g_Param_killbosscount	= 2							--2号：杀死任务boss怪的数量
x808016_g_Param_sceneid		= 3									--3号：当前副本任务的场景号
x808016_g_Param_teamid		= 4									--4号：接副本任务时候的队伍号
x808016_g_Param_time			= 5									--5号：完成副本所用时间(单位：秒)
																							--6号：具体副本事件脚本占用
																							--7号：具体副本事件脚本占用
--MisDescEnd
--************************************************************************

x808016_g_CopySceneType			= FUBEN_XIAOYAO1	--副本类型，定义在ScriptGlobal.lua里面
x808016_g_LimitMembers			= 1		--可以进副本的最小队伍人数
x808016_g_TickTime					= 5		--回调脚本的时钟时间（单位：秒/次）
x808016_g_LimitTotalHoldTime= 360	--副本可以存活的时间（单位：次数）,如果此时间到了，则任务将会失败
x808016_g_LimitTimeSuccess	= 500	--副本时间限制（单位：次数），如果此时间到了，任务完成
x808016_g_CloseTick					= 6		--副本关闭前倒计时（单位：次数）
x808016_g_NoUserTime				= 300	--副本中没有人后可以继续保存的时间（单位：秒）
x808016_g_Fuben_X						= 43	--进入副本的位置X
x808016_g_Fuben_Z						= 128	--进入副本的位置Z
x808016_g_Back_X						= 43	--源场景位置X
x808016_g_Back_Z						= 128	--源场景位置Z
x808016_g_Totalkillmonstercount	= 30	--需要杀死monster数量
x808016_g_Totalkillbosscount	= 1	--需要杀死Boss数量

--副本数据索引对照
x808016_g_keySD					= {}
x808016_g_keySD["typ"]	= 0		--设置副本类型
x808016_g_keySD["spt"]	= 1		--设置副本场景事件脚本号
x808016_g_keySD["tim"]	= 2		--设置定时器调用次数
x808016_g_keySD["scn"]	= 3		--设置副本入口场景号, 初始化
x808016_g_keySD["cls"]	= 4		--设置副本关闭标志, 0开放，1关闭
x808016_g_keySD["dwn"]	= 5		--设置离开倒计时次数
x808016_g_keySD["tem"]	= 6		--保存队伍号
x808016_g_keySD["x"]	= 7			--人物在入口场景中的x位置
x808016_g_keySD["z"]	= 8			--人物在入口场景中的z位置
x808016_g_keySD["killedmonsternum"]	= 9		--杀死喽啰的数量
x808016_g_keySD["killedbossnum"]	= 10		--杀死Boss的数量
x808016_g_keySD["mp"]	= 11		--记录当前副本的 门派

--接取任务的最低等级
x808016_g_minLevel			= 20
--需要配置小怪物
x808016_g_typMonster		= 749	--水鬼探子
--门派id
...
function x808016_OnEnumerate( sceneId, selfId, targetId )
	...
	if(MenPaiID ~= x808016_g_MenPaiID) then
		x808016_NotifyTip( sceneId, selfId, "我来是找逍遥麻烦的，你不是逍遥的弟子，请速速走开。" )
		return
	end

	local	lev	= GetLevel( sceneId, selfId )
	if lev < x808016_g_minLevel then
	  x808016_NotifyTip( sceneId, selfId, "你的等级太低了，根本不够我看的，还是20级之后再来找我吧。" )
		return
	end

	if LuaFnHasTeam( sceneId, selfId ) == 0 then
		x808016_NotifyTip( sceneId, selfId, "区区一个人就想来挑战我，我根本不屑与你动手。" )
		return
	end
	--PrintNum(3)

	if GetTeamSize( sceneId, selfId ) < x808016_g_LimitMembers then
	  x808016_NotifyTip( sceneId, selfId, "想要挑战我至少也得上来三个吧，就这点人？也太瞧不起我了。" )
	  return
	end
	--PrintNum(4)

	if LuaFnIsTeamLeader( sceneId, selfId ) == 0 then
		x808016_NotifyTip( sceneId, selfId, "想要挑战我？叫你们的队长来吧。" )
		return
	end
	--PrintNum(5)
	-- 取得玩家附近的队友数量（包括自己）
	...
	if nearteammembercount ~= LuaFnGetTeamSize( sceneId, selfId ) then
		x808016_NotifyTip( sceneId, selfId, "您队伍中有队员不在附近，请集合后再找我送你进入活动。" )
		return
	end
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x808016_g_ScriptId, "难道我还怕你不成……" ,10 ,0 )
  EndEvent( sceneId )
...
function x808016_CheckAccept( sceneId, selfId )
	local	MenPaiID	=	GetMenPai(sceneId, selfId)
	if(MenPaiID ~= x808016_g_MenPaiID) then
		x808016_NotifyTip( sceneId, selfId, "我来是找星宿麻烦的，你不是星宿的弟子，请速速走开。" )
		return 0;
	end
	...
	if IsHaveMission( sceneId, selfId, x808016_g_MissionId ) > 0 then
		...
		if copysceneid >= 0 and teamid == saveteamid then
			...
			if IsCanEnterCopyScene( copysceneid, GetHumanGUID( sceneId, selfId ) ) == 1 then
				...
				x808016_NotifyTip( sceneId, selfId, "任务失败，请放弃重新接取" )
...
function x808016_OnAbandon( sceneId, selfId )
	...
	--此时一定在副本中，可以获得入口场景号
	local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["scn"] )	--取得副本入口场景号
...
-- 	PrintStr("sdlf")
	...
	LuaFnSetSceneLoad_Map( sceneId, "xiaoyao_1.path" )
...
--	PrintStr("  "..x..z.." ")
  ...
  LuaFnSetCopySceneData_Param(sceneId, 13, mylevel)
	...
	if bRetSceneID > 0 then
		x808016_NotifyTip( sceneId, selfId, "副本创建成功！" )
	else
		x808016_NotifyTip( sceneId, selfId, "副本数量已达上限，请稍候再试！" )
...
function x808016_OnKillObject( sceneId, selfId, objdataId, objId )
	...
	local killedmonsternumber = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["killedmonsternum"] )			--杀死monster的数量
	local killedbossnumber = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["killedbossnum"] )			--杀死boss的数量
	...
	if(MonsterName == "喽啰")then
		...
		LuaFnSetCopySceneData_Param( sceneId, x808016_g_keySD["killedmonsternum"], killedmonsternumber )					--设置杀死monster的数量
		isBoss = 0
	...
	elseif ( MonsterName == "恶霸" ) then
		...
		LuaFnSetCopySceneData_Param( sceneId, x808016_g_keySD["killedbossnum"], killedbossnumber )					--设置杀死boss的数量
		isBoss = 1
	end
	...
	if (killedmonsternumber < x808016_g_Totalkillmonstercount ) or (killedbossnumber < x808016_g_Totalkillbosscount )then
		local strText = format( "已杀死喽啰： %d/%d 已杀死恶霸： %d/%d" , killedmonsternumber, x808016_g_Totalkillmonstercount, killedbossnumber, x808016_g_Totalkillbosscount )
		for i=0, num-1 do
			...
			if LuaFnIsObjValid( sceneId, humanObjId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, humanObjId ) == 1 then					--不在场景的不做此操作
				...
				misIndex = GetMissionIndexByID( sceneId, humanObjId, x808016_g_MissionId )					--取得任务数据索引值
				SetMissionByIndex( sceneId, humanObjId, misIndex, x808016_g_Param_killmonstercount, killedmonsternumber )	--设置任务数据
				SetMissionByIndex( sceneId, humanObjId, misIndex, x808016_g_Param_killbosscount, killedbossnumber )	--设置任务数据
			end
		end
	else
		...
		local strText = format( "已杀死喽啰： %d/%d 已杀死恶霸： %d/%d", x808016_g_Totalkillmonstercount, x808016_g_Totalkillmonstercount, x808016_g_Totalkillbosscount, x808016_g_Totalkillbosscount)
		local strText2 = format( "任务完成，将在%d秒后传送到入口位置", x808016_g_CloseTick * x808016_g_TickTime )
		...
		for i=0, num-1 do
			...
			if LuaFnIsObjValid( sceneId, humanObjId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, humanObjId ) == 1 then										--不在场景的不做此操作
				...
				SetMissionByIndex( sceneId, humanObjId, misIndex, x808016_g_Param_IsMissionOkFail, 1 )					--设置任务数据
				--完成副本所用时间
				SetMissionByIndex( sceneId, humanObjId, misIndex, x808016_g_Param_time, TickCount * x808016_g_TickTime )	--设置任务数据
	...
	for	i=0, numMem-1 do
		...
		if LuaFnIsCanDoScriptLogic( sceneId, member ) == 1 then			-- 处于可以执行逻辑的状态
			if IsHaveMission( sceneId, member, x808016_g_MissionId ) > 0 then
				...
				x808016_NotifyTip( sceneId, member, "你当前未接此任务" )
			end
		end
	end
end
...
function x808016_OnPlayerEnter( sceneId, selfId )
	if IsHaveMission( sceneId, selfId, x808016_g_MissionId ) == 0 then				--如果进入副本前删除任务，则直接传送回
		x808016_NotifyTip( sceneId, selfId, "你当前未接此任务" )
		local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["scn"] )		--取得副本入口场景号
		x = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["x"] )
		z = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["z"] )
		NewWorld( sceneId, selfId, oldsceneId, x, z )
...
--	end
	...
	if leaveFlag == 1 then
		...
		if leaveTickCount == x808016_g_CloseTick then										--倒计时间到，大家都出去吧
			local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["scn"] )	--取得副本入口场景号
		...
		elseif leaveTickCount < x808016_g_CloseTick then
			...
			local strText = format( "你将在%d秒后离开场景!", (x808016_g_CloseTick-leaveTickCount) * x808016_g_TickTime )
	...
	elseif TickCount == x808016_g_LimitTimeSuccess then
		...
		for	i=0, membercount-1 do
			...
			DelMission( sceneId, mems[i], x808016_g_MissionId )
				...
				x808016_NotifyTip( sceneId, mems[i], "任务时间到，完成!" )
				...
				--完成副本所用时间
				SetMissionByIndex( sceneId, mems[i], misIndex, x808016_g_Param_time, TickCount * x808016_g_TickTime )	--设置任务数据
			end
		end

		--设置副本关闭标志
		LuaFnSetCopySceneData_Param( sceneId, x808016_g_keySD["cls"], 1 )
	elseif TickCount == x808016_g_LimitTotalHoldTime then						--副本总时间限制到了
		--此处设置副本任务有时间限制的情况，当时间到后处理...
		for	i=0, membercount-1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 then
				DelMission( sceneId, mems[i], x808016_g_MissionId )				--任务失败,删除之
				x808016_NotifyTip( sceneId, mems[i], "任务失败，超时!" )
			end
		end
		...
		local oldteamid = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["tem"] )		--取得保存的队伍号
		local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, x808016_g_keySD["scn"] )	--取得副本入口场景号
		...
		for	i=0, membercount-1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and IsHaveMission( sceneId, mems[i], x808016_g_MissionId ) > 0 then
				if oldteamid ~= GetTeamId( sceneId, mems[i] ) then
					DelMission( sceneId, mems[i], x808016_g_MissionId )			--任务失败,删除之
					x808016_NotifyTip( sceneId, mems[i], "任务失败，你不在正确的队伍中!" )
