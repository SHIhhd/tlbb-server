...
x050221_g_CopySceneMap = "dumuchang.path"
...
x050221_g_TakeTimes = 99											-- 每天最多领取次数
--**********************************
-- 任务入口函数
--**********************************
...
function x050221_CheckAccept( sceneId, selfId, targetId )
	...
	-- end
	local takenTimes = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_XUANFOZHU)
	--5次
	...
	-- 所有心法达到45
	if ScriptGlobal_CheckAllXinfaLevel(sceneId, selfId, 45) == 0  then
		x050221_NotifyFailBox( sceneId, selfId, targetId, "#{LLFB_80816_5}" )
		return 0
	end
...
function x050221_OnAccept( sceneId, selfId, targetId )
	...
	-- 一天只能领 x050221_g_TakeTimes 次
	AddCampaignCountNum(sceneId,selfId,CAMPAIGN_XUANFOZHU)
...
function x050221_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050221_g_MissionId ) > 0 then					-- 有任务才可以走这里
		...
		for	i = 0, nearMemberCount - 1 do
			...
			if ScriptGlobal_CheckAllXinfaLevel(sceneId, memberID, 45) == 1 then
...
function x050221_OnCopySceneReady( sceneId, destsceneId )
	...
	for i = 1, validmembercount do
		if IsHaveMission( sceneId, members[i], x050221_g_MissionId ) > 0 then
			...
			if LuaFnIsObjValid( sceneId, members[i] ) == 1 and LuaFnIsCharacterLiving( sceneId, members[i] ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,members[i] ) == 1 then			-- 处于可以执行逻辑的状态
				--将任务的第1号数据设置为副本的场景号
...
function x050221_OnPlayerEnter( sceneId, selfId )
	--设置死亡后复活点位置
	SetPlayerDefaultReliveInfo(sceneId,selfId,"%50","%50","%10",sceneId, x050221_g_Fuben_X,x050221_g_Fuben_Z);
end
...
function x050221_OnDie( sceneId, objId, selfId )--这个selfId可能是宠物
	...
	for i = 1, getn( x050221_g_BossID ) do
		if szName == x050221_g_BossID[i] then
			...
			for j = 0, num - 1 do
				if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCharacterLiving( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,mems[i] ) == 1 then						-- 不在场景的不做此操作
		...
		for i = 0, num - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCharacterLiving( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,mems[i] ) == 1 then						-- 不在场景的不做此操作
				...
				if IsHaveMission( sceneId, mems[i], x050221_g_MissionId ) > 0 then
					...
					CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, mems[i],CAMPAIGN_XUANFOZHU)					
	...
	if TickCount == 1 then
		for i = 0, membercount - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCharacterLiving( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,mems[i] ) == 1 then
				for i = 1,5 do
					x050221_GenerateMonster(sceneId, mems[i], i)
				end
			end
		end
	end
...
-- 获取本副本的MonsterID
--**********************************
function x050221_GetFubenMonsterDataId(sceneId,selfId,nIndex)
	local BossYaoWangID = BossYaoWangIDTbl[1]
	if nIndex == 1 then --洪棘妖王
		...
		if BossYaoWangIDTbl[iniLevel] then
			BossYaoWangID = BossYaoWangIDTbl[iniLevel]
		end
	end
	return BossYaoWangID,1
end

--**********************************
--初始化....
--**********************************
function x050221_OnInit(sceneId, selfId)
end

--**********************************
--心跳....
--**********************************
function x050221_OnHeartBeat(sceneId, selfId, nTick)
end

--**********************************
--进入战斗....
--**********************************
function x050221_OnEnterCombat(sceneId, selfId, enmeyId)
	if GetName(sceneId,selfId) == x050221_g_BossID[1] then
		MonsterTalk(sceneId,selfId,"毒障泽地",x050221_g_BossTalk[1])
	end
	if GetName(sceneId,selfId) == x050221_g_BossID[2] then
		MonsterTalk(sceneId,selfId,"毒障泽地",x050221_g_BossTalk[2])
	end
	if GetName(sceneId,selfId) == x050221_g_BossID[3] then
		MonsterTalk(sceneId,selfId,"毒障泽地",x050221_g_BossTalk[3])
	end
	if GetName(sceneId,selfId) == x050221_g_BossID[4] then
		MonsterTalk(sceneId,selfId,"毒障泽地",x050221_g_BossTalk[4])
	end
	if GetName(sceneId,selfId) == x050221_g_BossID[5] then
		MonsterTalk(sceneId,selfId,"毒障泽地",x050221_g_BossTalk[5])
	end
	if GetName(sceneId,selfId) == x050221_g_BossYaoWang then
		MonsterTalk(sceneId,selfId,"毒障泽地",x050221_g_YaoWangTalk)
	end
end
...
--离开战斗....
--**********************************
function x050221_OnLeaveCombat(sceneId, selfId)
	--满血自己....
