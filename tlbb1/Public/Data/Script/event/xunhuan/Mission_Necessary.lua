...
--				end
  ...
  -- ========================================================
	...
	-- 封魂录埋点更新
	CallScriptFunction(791010, "AddPointData", sceneId, selfId, 4);
...
--end
	...
	BeginEvent(sceneId)
		...
		--投机取巧完成的，加入相关对话
		elseif isHelpFinish and isHelpFinish == 2 then
			AddText(sceneId, "#{TJQQ_90428_7}")
		end
	...
	DispatchMissionTips(sceneId,selfId)
	--周活跃数据埋点
	LuaFnFinishZhouHuoYueMission(sceneId,selfId,6)
	--英雄大会埋点
	if LuaFnHaveImpactOfSpecificDataIndex(sceneId,selfId,10541) == 1 then
		AddHerosReturnsRecoed(sceneId,selfId,19)
	end
	--师门任务统计
