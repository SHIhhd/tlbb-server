...
x050100_g_CopySceneMap = "songliao.path"
...
x050100_g_TakeTimes = 99											-- 每天最多领取次数
...
--	end
	...
	-- if nowDate ~= oldDate then
		-- takenTimes = 0
	-- end
	local takenTimes = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_YIGEDOUBUNENGPAO)
...
function x050100_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050100_g_MissionId ) == 0 then					-- 没有任务才可以走这里
		...
		-- 一天只能领 x050100_g_TakeTimes 次
		AddCampaignCountNum(sceneId,selfId,CAMPAIGN_YIGEDOUBUNENGPAO)
		-- local DayTimes
...
function x050100_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050100_g_MissionId ) > 0 then					-- 有任务才可以走这里
		...
		elseif mylevel < PlayerMaxLevel then
			...
			iniLevel = floor(PlayerMaxLevel / 10) * 10
		end
...
function x050100_OnKillObject( sceneId, selfId, objdataId, objId )						-- 参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	print("selfId ",selfId,"objId",objId)
	if IsHaveMission( sceneId, selfId, x050100_g_MissionId ) == 0 then
		return
	end
	...
	for i = 0, num - 1 do
		...
		if LuaFnIsObjValid( sceneId, humanObjId ) == 1 and LuaFnIsCharacterLiving( sceneId,humanObjId ) == 1 and LuaFnIsCanDoScriptLogic(sceneId,humanObjId ) == 1 then				-- 不在场景的不做此操作
			...
			if x050100_g_BossGroup == GroupID then
				...
				CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, humanObjId,CAMPAIGN_YIGEDOUBUNENGPAO)
				if i == 0 then
					--高光时刻结算
	...
	elseif TickCount == x050100_g_LimitTotalHoldTime then									-- 副本总时间限制到了
		...
		if flag > 0 then
			...
			elseif flag == 2 then
				...
				for	i = 0, membercount - 1 do
					PosX, PosZ = LuaFnGetWorldPos( sceneId, mems[i] )					-- 避免出现未找到队长的意外
					if LuaFnIsObjValid( sceneId, mems[i] ) == 1 
						and LuaFnIsTeamLeader( sceneId, mems[i] ) == 1 then
						break
					end
				end
...
function x050100_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x050100_CheckSubmit( sceneId, selfId ) == 1 then
		...
		--封魂录埋点更新
		CallScriptFunction(791010, "AddPointData", sceneId, selfId, 2);
	end
end
...
-- 获取本副本的MonsterID
--**********************************
function x050100_GetFubenMonsterDataId( sceneId, selfId,nIndex)
	--nIndex 1-6
	--余毒1个
	local mylevel = GetLevel(sceneId,selfId)
	local iniLevel = floor( mylevel/10 )
	if not x050100_g_Boss[iniLevel] then
		return x050100_g_Boss[getn(x050100_g_Boss)],1
	end
	return x050100_g_Boss[iniLevel],1
end
