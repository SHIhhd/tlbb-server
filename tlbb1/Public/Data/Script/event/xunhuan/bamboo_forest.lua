...
x050101_g_Boss = {4120,4121,4122,4123,4124,4125,4126,4127,4128,4129,34120,34121,34122,34123,34124,34125,34126,34127,34128,34129}
-- 任务奖励

--MisDescEnd
--************************************************************************

x050101_g_DemandKillGroup = { 1, 2 }		-- 1 ~ 2 号怪物对应的 GroupID 号，与 x050101_g_DemandKill 一一对应
...
x050101_g_CopySceneMap = "zhulin.path"
...
x050101_g_TakeTimes = 99											-- 每天最多领取次数
--**********************************
-- 任务入口函数
--**********************************
...
function x050101_CheckAccept( sceneId, selfId )
	...
	-- if nowDate ~= oldDate then
		-- takenTimes = 0
	-- end
	local takenTimes = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_CHUHAI)
...
function x050101_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050101_g_MissionId ) == 0 then					-- 没有任务才可以走这里
		...
		-- 一天只能领 x050101_g_TakeTimes 次
		AddCampaignCountNum(sceneId,selfId,CAMPAIGN_CHUHAI)
		-- local DayTimes
...
function x050101_OnPlayerEnter( sceneId, selfId )
	...
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x050101_g_EnterBuff, 0);
end
	...
	if leaveFlag == 1 then
		...
		for i = 1, num do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 then						-- 不在场景的不做此操作
				...
				CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, mems[i],CAMPAIGN_CHUHAI)
			end
		end
		--高光时刻结算
...
function x050101_PlayerExit(sceneId, selfId)
	LuaFnCancelSpecificImpact(sceneId, selfId, x050101_g_EnterBuff);
end
...
-- 获取本副本的MonsterID
--**********************************
function x050101_GetFubenMonsterDataId( sceneId, selfId,nIndex)
	--nIndex 1-6
	--余毒1个
	local mylevel = GetLevel(sceneId,selfId)
	local iniLevel = floor( mylevel/10 )
	if not x050101_g_Boss[iniLevel] then
		return x050101_g_Boss[getn(x050101_g_Boss)],1
	end
	return x050101_g_Boss[iniLevel],1
end
--**********************************
-- 对话窗口信息提示
--**********************************
