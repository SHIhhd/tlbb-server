...
function x006671_DisplayBonus(sceneId, missionIndex)
	...
	local awardMoney,awardMoneyJZ = TGetAwardMoney(missionIndex)
	if awardMoney > 0 then
		AddMoneyBonus( sceneId, awardMoney)
	elseif awardMoneyJZ > 0 then
		AddMoneyJZBonus( sceneId, awardMoneyJZ)
	end
end
--**********************************
--任务入口函数
...
function x006671_OnSubmit( sceneId, selfId, targetId, selectRadioId, missionIndex )
	...
	if IsHaveMission(sceneId,selfId,missionId) > 0 then
		...
		if ret > 0 then
			...
			elseif awardMoneyJZ > 0 then
				AddMoneyJZ(sceneId,selfId,awardMoneyJZ )
			end
