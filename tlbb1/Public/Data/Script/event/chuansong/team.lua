			...
			if teamFollowFlag == 1 then
				...
				for	i = 0, followMemberCount - 1 do
					...
					if followMembers[i] and followMembers[i] ~= -1 then
						...
						--地宫4特殊检查
						if newSceneId == SCENE_ID_DIGONG_4HD and followMembers[i] ~= selfId then
							if GetMissionDataEx(sceneId,followMembers[i],MDEX_ENTER_DIGONG_4HD_LASTTIME) == GetTime2Day() then
								x400900_NotifyFailTips(sceneId, selfId, "#{MJXZ_210510_201}")
								return
							end
							local selfMoney = GetMoney( sceneId, followMembers[i] )  +  GetMoneyJZ(sceneId, followMembers[i])
							if selfMoney < 200000 then
								x400900_NotifyFailTips(sceneId, selfId, "#{MJXZ_210510_243}")
								return
							end
						end
...
function x400900_CheckChangeScene(sceneId, selfId, newSceneId, posX, posY, minLevel, maxLevel)
	...
	--昆仑福地血量限制
	if newSceneId == SCENE_KUNLUNFUDI then
		if LuaFnGetMaxHp(sceneId,selfId) < 150000 then
			return 0, "此去凶险，你的血上限远远达不到15,000的要求，还需要好好沉淀啊。", GetName(sceneId, selfId).."的修为不达标，无法进入。";
		end
	end
	--【特殊PVP地图】限制最大血量
	if newSceneId == SCENE_JINGHU_HUANJING then
		if LuaFnGetMaxHp(sceneId,selfId) < 150000 then
			return 0, "幻境之地，诡谲云涌，少侠修为尚且不足，你的血上限远远达不到15,000的要求，还需要好好沉淀啊。", GetName(sceneId, selfId).."的修为不达标，无法进入。";
		end
	end
	return 1, "", "";
end
