...
x402276_g_CopySceneType = FUBEN_PIAOMIAOFENG	--副本类型，定义在ScriptGlobal.lua里面

x402276_g_TickTime		= 1				--回调脚本的时钟时间（单位：秒/次）
x402276_g_NoUserTime	= 300			--副本中没有人后可以继续保存的时间（单位：秒）
x402276_g_Fuben_X			= 124			--进入副本的位置X
x402276_g_Fuben_Z			= 164			--进入副本的位置Z
...
x402276_g_BOSSList =
{
	...
	["WuLaoDa_BOSS"]			= { DataID=9663, Title="万仙之首", posX=117, posY=49, Dir=11, BaseAI=27, AIScript=0, ScriptID=402279 },

	["ZhuoBuFan_BOSS"]		= { DataID=9664, Title="剑神", posX=121, posY=31, Dir=0, BaseAI=27, AIScript=0, ScriptID=402280 },
	["BuPingDaoRen_BOSS"]	= { DataID=9665, Title="蛟王", posX=129, posY=31, Dir=0, BaseAI=27, AIScript=261, ScriptID=402281 },
	...
	["LiQiuShui_BOSS"]		= { DataID=9666, Title="神秘女子", posX=125, posY=36, Dir=11, BaseAI=27, AIScript=0, ScriptID=402282 },
}
...
x402276_g_IDX_FuBenOpenTime		= 13	--副本建立的时间....
x402276_g_IDX_FuBenLifeStep		= 14	--副本生命期的step....(包括建立NPC....关闭倒计时提示....)
...
function x402276_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x402276_g_ScriptId, "初战缥缈峰", 10, 1 )
...
function x402276_CheckCanEnter( sceneId, selfId, targetId )
	...
	if GetName(sceneId,targetId) ~= "程青霜" then
		return 0, ""
	end
	--是否有队伍....
	...
	for i=0, NearTeamSize-1 do
		...
		local PlayerId = GetNearTeamMember( sceneId, selfId, i )
		local lastDayCount = GetCampaignCountNum(sceneId,PlayerId,CAMPAIGN_PIAOMIAOSMALL)
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. "#{XPMCZ_081108_1}"
		return 0, msg
...
function x402276_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "piaomiao.path")
...
function x402276_OnPlayerEnter( sceneId, selfId )
	local lastDayCount = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_PIAOMIAOSMALL)
	if lastDayCount >= 2 then
		--直接踢出副本给这个卡副本的。
	...
	--设置挑战过一次缥缈峰....
	AddCampaignCountNum(sceneId,selfId,CAMPAIGN_PIAOMIAOSMALL)
...
-- 获取本副本的MonsterID
--**********************************
function x402276_GetFubenMonsterDataId( sceneId, selfId,nIndex)
	--nIndex 1-6
	--哈大霸 桑土公 乌老大 任平生 李秋水
	
	--李秋水特写
	if nIndex == 5 then
		return x402276_g_FightBOSSList[6],1
	end
	return x402276_g_FightBOSSList[nIndex],0
end
