...
function x402282_OnDie( sceneId, selfId, killerId )
	...
	for i=0, GetNearTeamCount( sceneId, playerID )-1  do
		...
		if LuaFnIsObjValid(sceneId,nPlayerId) == 1 and LuaFnIsCanDoScriptLogic(sceneId,nPlayerId) == 1 then
			--周活跃数据埋点
			LuaFnFinishZhouHuoYueMission(sceneId,nPlayerId,13)
			--英雄大会埋点
			if LuaFnHaveImpactOfSpecificDataIndex(sceneId,nPlayerId,10541) == 1 then
				AddHerosReturnsRecoed(sceneId,nPlayerId,12)
			end
			CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, nPlayerId,CAMPAIGN_PIAOMIAOSMALL)
			--云侠赴战
			CallScriptFunction(890289,"OnFinish",sceneId, nPlayerId,CAMPAIGN_PIAOMIAOSMALL)
			--高光时刻结算
