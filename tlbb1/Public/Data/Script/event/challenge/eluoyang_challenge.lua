...
-- 副本场景
x806012_g_CopyScene = "leitai_2.path"
...
function x806012_OnPlayerEnter( sceneId, selfId )
	if LuaFnHaveImpactOfSpecificDataIndex(sceneId,selfId,x806012_g_PKViewBuff) == 1 then
		return
	end
...
function x806012_OnCopySceneTimer( sceneId, nowTime )
	...
	elseif tick > x806012_g_BeginTick then
		...
		for	i=0, membercount-1 do
			...
			if LuaFnIsObjValid( sceneId, playerId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, playerId ) == 1 then
				--不包含观战
				if LuaFnHaveImpactOfSpecificDataIndex(sceneId,playerId,x806012_g_PKViewBuff) == 0  then
