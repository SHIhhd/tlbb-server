...
-- 副本场景
x806014_g_CopyScene = "leitai_1.path"
...
function x806014_DoChallenge( sceneId, selfId, targetId )
	...
	LuaFnSetSceneLoad_Map( sceneId, x806014_g_CopyScene )						-- 地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
	...
	LuaFnSetCopySceneData_Param( sceneId, 0, x806014_g_CopySceneType )			-- 设置副本类型
	LuaFnSetCopySceneData_Param( sceneId, 1, x806014_g_ScriptId )				-- 将1号数据设置为副本场景事件脚本号
	...
	LuaFnSetSceneLoad_Area( sceneId, x806014_g_CopySceneExit )					-- 加载擂台出口
...
function x806014_OnCopySceneTimer( sceneId, nowTime )
	...
	if leaveFlag == 1 then																			-- 需要离开
		...
		if x806014_g_CloseTick <= leaveTickCount then
			...
			-- 通知剩余玩家副本即将关闭
			local membercount =	LuaFnGetCopyScene_HumanCount( sceneId )
			local strText =	format(	"擂台将在 %d 秒后关闭",	(x806014_g_CloseTick-leaveTickCount)*x806014_g_TickTime/1000 )
			BeginEvent(	sceneId	)
	...
	elseif tick > x806014_g_BeginTick then
		...
		if tick < x806014_g_BeginTick then
			strText = format( "比赛将在 %d 秒后开始", (x806014_g_BeginTick-tick)*x806014_g_TickTime/1000 )
		else
			strText = "比赛正式开始"
		end
