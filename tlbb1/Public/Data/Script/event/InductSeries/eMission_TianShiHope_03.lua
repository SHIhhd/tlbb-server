...
x500603_g_SceneID=0
x500603_g_AccomplishNPC_Name="周天师"
...
--任务是否已经完成
x500603_g_IsMissionOkFail	= 0		--任务参数的第0位

--任务文本描述
x500603_g_MissionName			= "天师的期待(3)"
--任务描述
...
x500603_g_Custom	= { {id="已升到32级",num=1} }
--MisDescEnd

--任务是否完成
x500603_g_Mission_IsComplete = 0		--任务参数的第0位
--是否达到等级的标记
x500603_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x500603_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x500603_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 1.洛阳NPC

x500603_g_AcceptMission_IDX		= 749	--接收任务索引
x500603_g_CompleteMission_IDX	= 750	--提交任务索引

--所拥有的事件ID列表
x500603_g_EventList	= {}

x500603_g_PlayerSlow_LVL					 = 30		-- 接受任务的最低等级

--奖励
x500603_g_MoneyBonus					=	1072
x500603_g_ExpBonus						= 42900
...
function x500603_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500603_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500603_g_Name then
			x500603_NotifyTip( sceneId, selfId, "接受任务失败" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x500603_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500603_g_AccomplishNPC_Name then
			x500603_NotifyTip( sceneId, selfId, "提交任务失败" )					
			return 0
		end
		-- 如果已经接了任务
...
--杀死怪物或玩家
--**********************************
function x500603_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
end
