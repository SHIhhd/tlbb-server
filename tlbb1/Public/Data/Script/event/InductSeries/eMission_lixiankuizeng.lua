...
x500619_g_givegift_diffday 		= 10        --超过10天给馈赠
...
function x500619_CheckUnlineGift( sceneId, selfId )
	...
	if (nDiffDay > x500619_g_givegift_diffday and CurLevel >= x500619_g_givegift_levellimit) then
		...
		LuaFnSendSystemMail( sceneId, GetName(sceneId, selfId), "#{LXJY_90226_02}" )   -- 发邮件
		--消息球
		--再战江湖	    由于您长时间没有上线，系统将自动补偿给您一些离线经验及道具补偿
		DispatchMsgBall(sceneId,selfId,41)
	else
...
function x500619_CalcGiveExpNum( sceneId, selfId )
	...
	local nGiveReady = 0
	if CurLevel >= 90 then
		nGiveReady = ExpInHan * 2 * ( nDiffDay - 10 )
	elseif CurLevel >= 30 then
		nGiveReady = ExpInHan * 2 * 1.5 * ( nDiffDay - 10 )
	end
