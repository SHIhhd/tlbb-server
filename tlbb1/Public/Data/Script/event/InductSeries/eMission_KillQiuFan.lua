--创建人[ QUFEI 2008-04-30 10:27 UPDATE BugID 34172 ]
--解决流失率引导任务事件脚本
--水牢引导任务1_太湖水寨
...
x500618_g_SceneID=1
x500618_g_AccomplishNPC_Name="呼延豹"
...
--任务是否已经完成
x500618_g_IsMissionOkFail	= 0		--任务参数的第0位

--任务文本描述
x500618_g_MissionName			= "太湖水寨"
--任务描述
...
x500618_g_Custom	= { {id="已杀死囚犯",num=20} }
--MisDescEnd

--任务是否完成
x500618_g_Mission_IsComplete = 0		--任务参数的第0位
--是否杀怪的标记
x500618_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x500618_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x500618_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 2.苏州NPC

x500618_g_AcceptMission_IDX		= 789	--接收任务索引
x500618_g_CompleteMission_IDX	= 790	--提交任务索引

--所拥有的事件ID列表
x500618_g_EventList	= {}

x500618_g_PlayerSlow_LVL					 = 45		-- 接受任务的最低等级
...
function x500618_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500618_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500618_g_Name then
			x500618_NotifyTip( sceneId, selfId, "接受任务失败" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x500618_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500618_g_AccomplishNPC_Name then
			x500618_NotifyTip( sceneId, selfId, "提交任务失败" )					
			return 0
		end
		-- 如果已经接了任务
...
--杀死怪物或玩家
--**********************************
function x500618_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
	...
	for i=0,num-1  do
		...
		if IsHaveMission(sceneId, humanObjId, x500618_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, x500618_g_Mission_IsComplete) <=0  then
				...
				BeginEvent(sceneId)
					local str = format("已杀死%s%d/%d", x500618_g_MonsterName, killedCount, x500618_g_KillMonsterCnt )						
					AddText(sceneId, str)
				EndEvent(sceneId)
