...
x500605_g_SceneID=0
x500605_g_AccomplishNPC_Name="周天师"
...
--任务是否已经完成
x500605_g_IsMissionOkFail	= 0		--任务参数的第0位

--任务文本描述
x500605_g_MissionName			= "天师的期待(4)"
--任务描述
...
x500605_g_Custom	= { {id="已升到35级",num=1} }
--MisDescEnd

--任务是否完成
x500605_g_Mission_IsComplete = 0		--任务参数的第0位
--是否达到等级的标记
x500605_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x500605_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x500605_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 1.洛阳NPC
...
x500605_g_PlayerSlow_LVL					 = 32		-- 接受任务的最低等级

--奖励
x500605_g_MoneyBonus					=	1161
x500605_g_ExpBonus						= 46470
...
function x500605_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500605_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500605_g_Name then
			x500605_NotifyTip( sceneId, selfId, "接受任务失败" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x500605_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500605_g_AccomplishNPC_Name then
			x500605_NotifyTip( sceneId, selfId, "提交任务失败" )					
			return 0
		end
		-- 如果已经接了任务
