...
x500611_g_SceneID=1
x500611_g_AccomplishNPC_Name="钱宏宇"
...
--任务是否已经完成
x500611_g_IsMissionOkFail	= 0		--任务参数的第0位

--任务文本描述
x500611_g_MissionName			= "干掉山寨大王"
--任务描述
...
x500611_g_Custom	= { {id="已杀死山寨大王",num=1} }
--MisDescEnd

--任务是否完成
x500611_g_Mission_IsComplete = 0		--任务参数的第0位
--是否杀怪的标记
x500611_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x500611_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x500611_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 2.苏州NPC

x500611_g_AcceptMission_IDX		= 765	--接收任务索引
x500611_g_CompleteMission_IDX	= 766	--提交任务索引

--所拥有的事件ID列表
x500611_g_EventList	= {}

x500611_g_PlayerSlow_LVL					 = 38		-- 接受任务的最低等级

--奖励
x500611_g_MoneyBonus					=	9475
x500611_g_ExpBonus						= 379020
x500611_g_ItemBonus_List 	 	  = { id=30505701,num=1 }

--副本场景ID_野外匪寨
x500611_g_TargetSceneId				= 48
--怪物名称
x500611_g_MonsterName					=	"山寨大王"
--要求的杀怪数量
...
function x500611_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500611_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500611_g_Name then
			x500611_NotifyTip( sceneId, selfId, "接受任务失败" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x500611_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500611_g_AccomplishNPC_Name then
			x500611_NotifyTip( sceneId, selfId, "提交任务失败" )					
			return 0
		end
		-- 如果已经接了任务
...
--杀死怪物或玩家
--**********************************
function x500611_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
	...
	for i=0,num-1  do
		...
		if IsHaveMission(sceneId, humanObjId, x500611_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, x500611_g_Mission_IsComplete) <=0  then
				...
				BeginEvent(sceneId)
					local str = format("已杀死%s%d/%d", x500611_g_MonsterName, killedCount, x500611_g_KillMonsterCnt )						
					AddText(sceneId, str)
				EndEvent(sceneId)
				...
				--周活跃数据埋点
				LuaFnFinishZhouHuoYueMission(sceneId,humanObjId,10)
			end
		end
	end
