...
x500607_g_SceneID=0
x500607_g_AccomplishNPC_Name="周天师"
...
--任务是否已经完成
x500607_g_IsMissionOkFail	= 0		--任务参数的第0位

--任务文本描述
x500607_g_MissionName			= "江湖宵小"
--任务描述
...
x500607_g_Custom	= { {id="已杀死恶霸",num=1} }
--MisDescEnd

--任务是否完成
x500607_g_Mission_IsComplete = 0		--任务参数的第0位
--是否杀怪的标记
x500607_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x500607_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x500607_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 1.洛阳NPC
...
x500607_g_PlayerSlow_LVL					 = 35		-- 接受任务的最低等级
...
function x500607_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500607_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500607_g_Name then
			x500607_NotifyTip( sceneId, selfId, "接受任务失败" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x500607_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500607_g_AccomplishNPC_Name then
			x500607_NotifyTip( sceneId, selfId, "提交任务失败" )					
			return 0
		end
		-- 如果已经接了任务
	...
	for i=0,num-1  do
		...
		if IsHaveMission(sceneId, humanObjId, x500607_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, x500607_g_Mission_IsComplete) <=0  then
				...
				BeginEvent(sceneId)
					local str = format("已杀死%s%d/%d", x500607_g_MonsterName, killedCount, x500607_g_KillMonsterCnt )						
					AddText(sceneId, str)
				EndEvent(sceneId)
