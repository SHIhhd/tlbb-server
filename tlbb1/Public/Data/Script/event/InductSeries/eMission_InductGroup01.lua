--创建人[ QUFEI 2008-04-30 10:27 UPDATE BugID 34164 ]
--解决流失率引导任务事件脚本
--组队引导任务2_练习组队
...
x500600_g_SceneID=0
x500600_g_AccomplishNPC_Name="周天师"
...
--任务是否已经完成
x500600_g_IsMissionOkFail	= 0		--任务参数的第0位

--任务文本描述
x500600_g_MissionName			= "练习组队"
--任务描述
...
x500600_g_Custom	= { {id="组成一个二人以上的队伍",num=1} }
--MisDescEnd

--任务是否完成
x500600_g_Mission_IsComplete = 0		--任务参数的第0位
--是否组队的标记
x500600_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x500600_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x500600_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 1.洛阳NPC

x500600_g_AcceptMission_IDX		= 743	--接收任务索引
x500600_g_CompleteMission_IDX	= 744	--提交任务索引

--所拥有的事件ID列表
x500600_g_EventList	= {}

x500600_g_PlayerSlow_LVL					 = 26		-- 接受任务的最低等级

--奖励
x500600_g_MoneyBonus					=	1800
x500600_g_ExpBonus						= 70000
...
function x500600_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500600_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500600_g_Name then
			x500600_NotifyTip( sceneId, selfId, "接受任务失败" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x500600_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500600_g_AccomplishNPC_Name then
			x500600_NotifyTip( sceneId, selfId, "提交任务失败" )					
			return 0
		end
		-- 如果已经接了任务
...
--杀死怪物或玩家
--**********************************
function x500600_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
end
