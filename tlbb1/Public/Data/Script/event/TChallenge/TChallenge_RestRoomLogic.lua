...
x890025_g_ScriptId = 890025

x890025_g_GlobalChallengeScript = 806010

-- 副本场景
x890025_g_CopyScene = "leitai_3.path"

-- 副本出口
x890025_g_CopySceneExit = "leitai_3_area.ini"

-- 副本登陆地点
x890025_g_Pos = { x=31, z=32, offset=3.53 }

-- 副本无人时关闭时间 (毫秒)
x890025_g_CopySceneCloseTime = 3000

-- 副本定时器触发周期 (毫秒)
x890025_g_TickTime = 5000

-- 擂台开始时间
x890025_g_BeginTick = 3

-- 副本只剩下一方时的关闭周期 ( 单位为定时器触发周期 )
x890025_g_CloseTick = 3

-- 副本Pvp规则
x890025_g_PvpRuler = 9	--zchw

-- 副本类型号
x890025_g_CopySceneType = FUBEN_PVP_LEITAI

-- 挑战双方阵营号
x890025_g_Camp = { self=10, target=11 }

-- 挑战结束以后回到的位置
x890025_g_BackPos = { sceneId=SCENE_TCHALLENGE, x=96, z=104 }

-- 无敌 BUFF ~~~
x890025_g_Buff = 112

--竞技场观战buff
x890025_g_PKViewBuff = 2693	

x890025_g_BuffID_ClearChgBodyBuff = 84
-- 让 selfId (及队友) 挑战 targetId (及队友)
function x890025_DoChallenge( sceneId, selfId, targetId )

	-- 先创建一个副本，如果创建成功，则继续，否则返回
	local CreatorGUID = LuaFnObjId2Guid( sceneId, selfId )
	local TargetGUID = LuaFnObjId2Guid( sceneId, targetId )
	LuaFnSetSceneLoad_Map( sceneId, x890025_g_CopyScene )						-- 地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
	LuaFnSetCopySceneData_TeamLeader( sceneId, CreatorGUID )
	LuaFnSetCopySceneData_NoUserCloseTime( sceneId, x890025_g_CopySceneCloseTime )
	LuaFnSetCopySceneData_PvpRuler( sceneId, x890025_g_PvpRuler )
	LuaFnSetCopySceneData_Timer( sceneId, x890025_g_TickTime )
	LuaFnSetCopySceneData_Param( sceneId, 0, x890025_g_CopySceneType )			-- 设置副本类型
	LuaFnSetCopySceneData_Param( sceneId, 1, x890025_g_ScriptId )				-- 将1号数据设置为副本场景事件脚本号
	...
	LuaFnSetSceneLoad_Area( sceneId, x890025_g_CopySceneExit )					-- 加载擂台出口

	local CopySceneID = LuaFnCreateCopyScene( sceneId )					-- 初始化完成后调用创建副本函数
	if CopySceneID <= 0 then
		BeginEvent( sceneId )
			AddText( sceneId, "#{TFXC_20220921_20}" )
		EndEvent( sceneId )
...
function x890025_GetSelfTeamId( sceneId )
	return LuaFnGetCopySceneData_Param( sceneId, 6 ) / 10
end

function x890025_GetTargetTeamId( sceneId )
	return LuaFnGetCopySceneData_Param( sceneId, 7 ) / 10
end

function x890025_GetSelfMembersCount( sceneId )
...
function x890025_ModifySelfMembersCount( sceneId, count )
	local selfInfo = LuaFnGetCopySceneData_Param( sceneId, 6 )
...
function x890025_GetTargetMembersCount( sceneId )
...
function x890025_ModifyTargetMembersCount( sceneId, count )
	local TargetInfo = LuaFnGetCopySceneData_Param( sceneId, 7 )
...
function x890025_CalcPosOffset()
	local offset = x890025_g_Pos.offset * random(100) / 100
...
--副本事件
--**********************************
function x890025_OnCopySceneReady( sceneId, destsceneId )
	...
	for i=0, membersCount-1 do
		...
		if LuaFnIsCanDoScriptLogic( sceneId, members[i] ) == 1 then
			NewWorld( sceneId, members[i], destsceneId, x890025_g_Pos.x + x890025_CalcPosOffset(), x890025_g_Pos.z + x890025_CalcPosOffset())
		end
	end
end

-- 获得自己站在哪方的信息，1: 挑战方，2: 被挑战方
function x890025_GetMySide( sceneId, selfId )
	...
	elseif MyGUID == targetGUID then
		...
		local selfTeamId = x890025_GetSelfTeamId( sceneId )
		local targetTeamId = x890025_GetTargetTeamId( sceneId )
...
--有玩家进入副本事件
--**********************************
function x890025_OnPlayerEnter( sceneId, selfId )
	if LuaFnHaveImpactOfSpecificDataIndex(sceneId,selfId,x890025_g_PKViewBuff) == 1 then
		return
	end
	...
	local MySide = x890025_GetMySide( sceneId, selfId )
	if MySide == 1 then
		SetUnitCampID( sceneId, selfId, selfId, x890025_g_Camp.self )
		SetPvpAuthorizationFlagByID(sceneId, selfId, 2, 1) --2是竞技授权标记
	--	x890025_ModifySelfMembersCount( sceneId, 1 )
	else
		SetUnitCampID( sceneId, selfId, selfId, x890025_g_Camp.target )
		SetPvpAuthorizationFlagByID(sceneId, selfId, 2, 1) --2是竞技授权标记
	--	x890025_ModifyTargetMembersCount( sceneId, 1 )
	end

	-- 加上 15 秒的 buff，也许应该放在 NewWorld 以前，主要害怕如果 NewWorld 失败可能会有一些副作用
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x890025_g_Buff, 0)

	-- 设置默认还魂点为进入场景的擂台
	SetPlayerDefaultReliveInfo( sceneId, selfId, "%10", -1, "0", x890025_g_BackPos.sceneId, x890025_g_BackPos.x, x890025_g_BackPos.z )
	-- 进入校场副本的玩家要清除变身buff，防止不能操作被打死
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x890025_g_BuffID_ClearChgBodyBuff, 0)
end

-- 副本出口离开调用这个函数
function x890025_LeaveScene( sceneId, selfId )
	-- 统计剩余人数
	--local MySide = x890025_GetMySide( sceneId, selfId )
	--if MySide == 1 then
	--	x890025_ModifySelfMembersCount( sceneId, -1 )
	...
	CallScriptFunction( (400900), "TransferFunc", sceneId, selfId, x890025_g_BackPos.sceneId, x890025_g_BackPos.x, x890025_g_BackPos.z )
end

--**********************************
--有玩家在副本中死亡事件
--**********************************
function x890025_OnHumanDie( sceneId, selfId, killerId )
	--周活跃数据埋点
	LuaFnFinishZhouHuoYueMission(sceneId,selfId,23)
	LuaFnFinishZhouHuoYueMission(sceneId,killerId,23)
	--英雄大会埋点
	if LuaFnHaveImpactOfSpecificDataIndex(sceneId,selfId,10541) == 1 then
		AddHerosReturnsRecoed(sceneId,selfId,4)
	end
	if LuaFnHaveImpactOfSpecificDataIndex(sceneId,killerId,10541) == 1 then
		AddHerosReturnsRecoed(sceneId,killerId,4)
	end
end
...
--副本场景定时器事件
--**********************************
function x890025_OnCopySceneTimer( sceneId, nowTime )
	-- 副本时钟设置
	...
	if leaveFlag == 1 then																			-- 需要离开
		...
		if x890025_g_CloseTick <= leaveTickCount then
			-- 将当前副本场景里的所有人传送回原来进入时候的场景
			local membercount = LuaFnGetCopyScene_HumanCount( sceneId )
			for	i=0, membercount-1 do
				local playerId = LuaFnGetCopyScene_HumanObjId( sceneId, i )
				if LuaFnIsObjValid( sceneId, playerId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, playerId ) == 1 then
					x890025_LeaveScene( sceneId, playerId )
				end
			end
		else
			-- 通知剩余玩家副本即将关闭
			local membercount = LuaFnGetCopyScene_HumanCount( sceneId )
	  		local strText = ScriptGlobal_Format( "#{TFXC_20220921_27}", (x890025_g_CloseTick-leaveTickCount)*x890025_g_TickTime/1000 )
	  		BeginEvent( sceneId )
	...
	elseif tick > x890025_g_BeginTick then
		...
		for	i=0, membercount-1 do
			...
			if LuaFnIsObjValid( sceneId, playerId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, playerId ) == 1 then
				--不包含观战
				if LuaFnHaveImpactOfSpecificDataIndex(sceneId,playerId,x890025_g_PKViewBuff) == 0  then
					if x890025_GetMySide( sceneId, playerId ) == 1 then
		...
		if tick < x890025_g_BeginTick then
			strText = format( "比赛将在 %d 秒后开始", (x890025_g_BeginTick-tick)*x890025_g_TickTime/1000 )
		else
			strText = "比赛正式开始"
		end
