--装备强化
--脚本号
x809262_g_ScriptId = 809262

x809262_g_QianghualuId = 30900045

x809262_g_QianghualuTime = 11
gem_index = 30900045

x809262_g_EquipName = {
	[0]		="武器",
	[1]		="帽子",
	[2]		="衣服",
	[3]		="手套",
	[4]		="鞋"  ,
	[5]		="腰带",
	[6]		="戒指",
	[7]	    ="项鍊",
	[8]		="骑乘",
	[9]		="行囊",
	[10]	="箱格",
	[11]	="戒指",
	[12]	="护符",
	[13]    ="护符",
	[14]	="护腕",
	[15]    ="护肩",
	[16]	="时装",
	[17]  ="暗器",
	[18]  ="武魂",
	-- [19]  ="龙纹",
	-- [20]  ="令牌",
	-- [21]	="豪侠印"
}

--**********************************
--列举事件
--**********************************
function x809262_OnEnumerate( sceneId, selfId, targetId )

end


--**********************************
--装备强化检查
--**********************************
function x809262_FinishEnhance( sceneId, selfId, itemidx1, itemidx2 )
	local ret = LuaFnIsItemLocked( sceneId, selfId, itemidx1 )
	
	if ret ~= 0 then
		BeginEvent(sceneId)
		AddText(sceneId,"该装备不可用。");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	
	ret = LuaFnIsItemAvailable( sceneId, selfId, itemidx2 )
	if ret ~= 1 then
		BeginEvent(sceneId)
		--AddText(sceneId,"该强化精华不可用。");
		AddText(sceneId,"该强化精华或强化露不可用。");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	
	equip_level = GetBagItemLevel( sceneId, selfId, itemidx1 )
	gem_index = LuaFnGetItemTableIndexByIndex( sceneId, selfId, itemidx2 )
	if equip_level < 40 then
	
		if gem_index ~= 30900005 then
			--低级强化精华
			str	= format( "该装备强化需要%s。", GetItemName( sceneId, 30900005 ) )
			BeginEvent(sceneId)
			AddText(sceneId,str);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		end
	else

		if gem_index ~= 30900006 and gem_index ~= x809262_g_QianghualuId then
			--高级强化精华
			str	= format( "该装备强化需要%s或%s。", GetItemName( sceneId, 30900006 ), GetItemName( sceneId, x809262_g_QianghualuId ) )
			BeginEvent(sceneId)
			AddText(sceneId,str);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		end
	end

	--检查是否能够强化 
	ret, arg0 = LuaFnEquipEnhanceCheck( sceneId, selfId, itemidx1, itemidx2 )
	local text="装备强化成功！"
	-- zchw
	equip_index = tonumber(LuaFnGetItemTableIndexByIndex( sceneId, selfId, itemidx1 )); 
	if (equip_index ==10423024) or (equip_index == 10422016) then --重楼玉，重楼戒
		if tonumber(gem_index) == 30900006 then --强化精华
			bindStatus = tonumber(LuaFnGetItemBindStatus(sceneId, selfId, itemidx2));
			if bindStatus == 1 then --绑定
				text = "该装备不能用已绑定的天罡强化精华强化！";
				BeginEvent(sceneId);
				AddText(sceneId,text);
				EndEvent(sceneId);
				DispatchMissionTips(sceneId,selfId);
				return
			end
		end
	end
	if ret == 0 then
		--强化消耗检查成功
		x809262_DoFinishEnhance( sceneId, selfId, itemidx1*1000+itemidx2)
		return
	end
	
	if ret == -1 then
		text="未知错误。"
	end
	
	if ret == -2 then
		text="装备不可用。"
	end
	
	if ret == -3 then
		--text="强化精华不可用。"
		text="强化精华或强化露不可用。"
	end
	
	if ret == -4 then
		text="该装备的强化等级已经最大。"
	end

	if ret == -5 then
		text="强化该装备需要#{_EXCHG%d}，您身上的现金不足。" --zchw
		text=format( text, arg0 )
	end
	
	if ret == -7 then
		text="该装备不能被强化。"
	end

	if ret == -6 then
		--需要确认界面
		--text="强化失败之后，强化等级将会变成"..tostring(arg0).."，是否继续强化？"
		--BeginUICommand(sceneId)
			--UICommand_AddInt(sceneId,x809262_g_ScriptId);
			--UICommand_AddInt(sceneId,targetId);
			--UICommand_AddInt(sceneId,itemidx1*1000+itemidx2);
			--UICommand_AddString(sceneId,"DoFinishEnhance");
			--UICommand_AddString(sceneId,text);
			--EndUICommand(sceneId)
		--DispatchUICommand(sceneId,selfId, 24)
		x809262_DoFinishEnhance( sceneId, selfId, itemidx1*1000+itemidx2)
		return
	end
	
	BeginEvent(sceneId)
	AddText(sceneId,text)
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end

--**********************************
--装备强化
--**********************************
function x809262_DoFinishEnhance( sceneId, selfId, index )
	local itemidx1 = floor(index / 1000)
	local itemidx2 = mod(index , 1000)
	local text="装备强化成功！"
	local gem_index = LuaFnGetItemTableIndexByIndex( sceneId, selfId, itemidx2 )
	--强化
	local ret,arg0 = LuaFnEquipEnhance( sceneId, selfId, itemidx1, itemidx2 )

	if ret == 0 then
		if arg0 >= 2 then
			local	szTranItm1	= GetBagItemTransfer( sceneId, selfId, itemidx1 )
			local	szTranItm2	= GetBagItemTransfer( sceneId, selfId, itemidx2 )
			local	szMsg				= format( "#W#{_INFOUSR%s}#{EQ_1}#{_INFOMSG%s}#{EQ_2}%d#{EQ_3}#{_INFOMSG%s}#{EQ_4}",
														LuaFnGetName( sceneId, selfId ), szTranItm2, arg0, szTranItm1 )
			--公告精简，小于5级的强化，不发公告
			if (arg0 >= 5) then
				AddGlobalCountNews( sceneId, szMsg )
			end
		end
		if x809262_g_QianghualuId == gem_index then
			local r, t = LuaFnEraseItemTimes( sceneId, selfId, itemidx2, x809262_g_QianghualuTime )
			--PrintNum(r);
			--PrintNum(t)
			
			BeginEvent(sceneId)
			--local str = "天罡强化露剩余使用次数%d"
			local	szMsg				= format( "天罡强化露剩余使用次数%d/%d",
														tonumber(t), tonumber(x809262_g_QianghualuTime) );
			AddText(sceneId,szMsg);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		else
			--LuaFnEraseItem( sceneId, selfId, itemidx2 )
			--因材料叠加物品扣除处理修改规则
			LuaFnDelAvailableItem(sceneId,selfId,gem_index,1)
		end
		
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 49, 0);
	end

	if ret == -1 then
		text="未知错误。"
	end
	
	if ret == -2 then
		text="装备不可用。"
	end
	
	if ret == -3 then
		text="强化精华不可用。"
	end
	
	if ret == -4 then
		text="该装备的强化等级已经最大。"
	end

	if ret == -5 then
		text="强化该装备需要#{_EXCHG%d}，您身上的现金不足。" --zchw
		text=format( text, arg0 )
	end

	if ret == -6 then
		text="好可惜啊您的装备强化失败了,别灰心!!!"
		
		if x809262_g_QianghualuId == gem_index then
			local r, t = LuaFnEraseItemTimes( sceneId, selfId, itemidx2, x809262_g_QianghualuTime )
			--PrintNum(r);
			--PrintNum(t)
			
			BeginEvent(sceneId)
			local	szMsg				= format( "天罡强化露剩余使用次数%d/%d",
														tonumber(t), tonumber(x809262_g_QianghualuTime) );
			AddText(sceneId,szMsg);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
	
		else
			--LuaFnEraseItem( sceneId, selfId, itemidx2 )
			--因材料叠加物品扣除处理修改规则
			LuaFnDelAvailableItem(sceneId,selfId,gem_index,1)
		end
		--LuaFnEraseItem( sceneId, selfId, itemidx2 )
	end

	BeginEvent(sceneId)
	AddText(sceneId,text);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
--**********************************
--元宝快速强化
--**********************************
--这里数据先乱编
x809262_g_QuickCostL = {
	[0] = 9240,		--准确
	[1] = 8040,
	[2] = 6840,
	[3] = 5640,
	[4] = 4840,
	[5] = 4040,
	[6] = 3240,
	[7] = 1640,
	[8] = 840,
}
x809262_g_QuickCostH = {
	[0] = 288888,--28542,	--准确
	[1] = 249422,
	[2] = 213422,
	[3] = 177422,
	[4] = 153422,
	[5] = 129422,
	[6] = 105422,
	[7] = 81422,
	[8] = 57422,
}
function x809262_DoQuickQiangHuaByYB( sceneId, selfId,nPos,isOk)
	local nItemId = LuaFnGetItemTableIndexByIndex( sceneId, selfId, nPos )
	local equipType = GetItemEquipPoint(nItemId )
	if equipType == -1 then
		x809262_TipBox( sceneId, selfId, "#{QHTS_200710_03}" )
		return
	end
	local nCurLevel  = LuaFnGetEquipEnhanceLevel(sceneId,selfId,nPos)
	if nCurLevel == 9 then
		x809262_TipBox( sceneId, selfId, "#{QHTS_200710_05}" )
		return
	end
	local nCheckRet = LuaFnEquipEnhanceCheck( sceneId, selfId, nPos, nPos )
	if nCheckRet == -7 then
		x809262_TipBox( sceneId, selfId, "#{QHTS_200710_10}" )
		return
	end
	local nNeedYuanbao = 0
	local equip_level = GetBagItemLevel( sceneId, selfId, nPos )
	if equip_level < 40 then
		--低级强化精华
		nNeedYuanbao = x809262_g_QuickCostL[nCurLevel]
	else
		--高级强化精华
		nNeedYuanbao = x809262_g_QuickCostH[nCurLevel]
	end
	local ItemName = GetItemName(sceneId, nItemId)
	if isOk ~= 1 then
		BeginUICommand( sceneId )
			UICommand_AddInt( sceneId, nPos )
			UICommand_AddString(sceneId,ScriptGlobal_Format("#{QHTS_200710_04}",ItemName,tostring(nCurLevel),tostring(nNeedYuanbao)))
		EndUICommand( sceneId )
		DispatchUICommand( sceneId, selfId, 80926201 )	
		return
	end
	local nCurYuanbao = YuanBao(sceneId,selfId,selfId,3,0)
	if nCurYuanbao < nNeedYuanbao then
		x809262_TipBox(sceneId, selfId, "#{ZSYB_151109_03}" );
		return
	end
	if LuaFnIsCanYuanBaoCost(sceneId,selfId,nNeedYuanbao) ~= 1 then
		x809262_TipBox(sceneId, selfId, "#{ZSYB_151109_11}" );
		return
	end
	if LuaFnCostYuanBao(sceneId,selfId,nNeedYuanbao,809262,"DoQuickQiangHuaByYB",0,0) ~= 1 then
		return
	end
	--设置等级为9
	LuaFnSetEquipEnhanceLevel(sceneId,selfId,nPos,9)
	
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 49, 0);
	x809262_TipBox( sceneId, selfId,ScriptGlobal_Format("#{QHTS_200710_07}",ItemName))
	
	local	szTranItm	= GetBagItemTransfer( sceneId, selfId, nPos )
	local	szMsg = ScriptGlobal_Format("#{QHTS_200710_09}",LuaFnGetName( sceneId, selfId ),szTranItm)
	AddGlobalCountNews( sceneId, szMsg )
end

--**********************************
--装备强化转移
--**********************************
function x809262_FinishMoveEnhance( sceneId, selfId,nPosSource,nPosTarget)
	if nPosTarget == nPosSource then
		return
	end
	local ret = LuaFnIsItemLocked( sceneId, selfId, nPosSource )
	if ret ~= 0 then
		x809262_TipBox( sceneId, selfId, "#{ZBQH_130521_13}" )
		return
	end
	ret = LuaFnIsItemAvailable( sceneId, selfId, nPosTarget )
	if ret ~= 1 then
		x809262_TipBox( sceneId, selfId, "#{ZBQH_130521_20}" )
		return
	end
	local nItemSourceId = LuaFnGetItemTableIndexByIndex( sceneId, selfId, nPosSource )
	local nItemTargetId = LuaFnGetItemTableIndexByIndex( sceneId, selfId, nPosTarget )
	local equipType1 = GetItemEquipPoint(nItemSourceId )
	local equipType2 = GetItemEquipPoint(nItemTargetId )
	local curlevel1  = LuaFnGetEquipEnhanceLevel(sceneId,selfId,nPosSource)
	local curlevel2  = LuaFnGetEquipEnhanceLevel(sceneId,selfId,nPosTarget)
	local nItemSourceLev = GetBagItemLevel( sceneId, selfId, nPosSource )
	local nItemTargetLev = GetBagItemLevel( sceneId, selfId, nPosTarget )
	local leftItemName = GetItemName(sceneId, nItemSourceId)
	if equipType1 == -1 or equipType2 == -1 then
		return
	end
	if equipType1 >= 19 and equipType1 <= 24 then  --灵武的限制
		x809262_TipBox( sceneId, selfId,"#{SZXT_221216_213}")
		return
	end
	if equipType2 >= 19 and equipType2 <= 24 then  --灵武的限制
		x809262_TipBox( sceneId, selfId,"#{SZXT_221216_213}")
		return
	end
	if equipType1 ~= equipType2 then
		if nItemSourceLev < 40 then
			x809262_TipBox( sceneId, selfId, ScriptGlobal_Format("#{ZBQH_130521_17}",leftItemName, x809262_g_EquipName[nItemSourceLev]))
			return
		end
		if nItemSourceLev >= 40 then
			x809262_TipBox( sceneId, selfId, ScriptGlobal_Format("#{ZBQH_130521_18}",leftItemName, x809262_g_EquipName[nItemSourceLev]))
			return
		end
	end
	if nItemSourceLev < 40 and nItemTargetLev >= 40 then
		x809262_TipBox( sceneId, selfId, ScriptGlobal_Format("#{ZBQH_130521_17}",leftItemName, x809262_g_EquipName[nItemSourceLev]))
		return
	end
	if nItemSourceLev >= 40 and nItemTargetLev < 40 then
		x809262_TipBox( sceneId, selfId, ScriptGlobal_Format("#{ZBQH_130521_18}",leftItemName, x809262_g_EquipName[nItemSourceLev]))
		return
	end
	if curlevel1 <= curlevel2 then
		x809262_TipBox( sceneId, selfId, ScriptGlobal_Format("#{ZBQH_130521_19}",curlevel1))
		return
	end
	local NeedMoney = curlevel1*5
	if NeedMoney > 100 then
		NeedMoney = 100
	end
	local nMoneyJZ = GetMoneyJZ(sceneId,selfId)
	local nMoneyJB = GetMoney(sceneId,selfId)
	local nMoneySelf = nMoneyJZ + nMoneyJB
	if nMoneySelf < NeedMoney*10000 then
		x809262_TipBox( sceneId, selfId,"#{ZBQH_130521_24}")
		return
	end
	--扣钱
	LuaFnCostMoneyWithPriority( sceneId, selfId,  NeedMoney*10000 );
	--绑定处理
	if tonumber(LuaFnGetItemBindStatus(sceneId, selfId, nPosSource)) == 1 then
		LuaFnItemBind(sceneId,selfId,nPosTarget)
	end
	--执行转移
	LuaFnEquipEnhanceTrans(sceneId,selfId,nPosSource,nPosTarget)
	
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 49, 0);
	x809262_TipBox( sceneId, selfId, "#{ZBQH_130521_25}")
end

--**********************************
--屏幕中心提示
--**********************************
function x809262_TipBox(sceneId,selfId,nMsg)
	BeginEvent(sceneId)
		AddText(sceneId,nMsg);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
