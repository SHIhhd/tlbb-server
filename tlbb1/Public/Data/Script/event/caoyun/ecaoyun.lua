...
--**********************************
--购买或卖出某个盐，铁，米
--ClientInterfaceCarriageCarriage.lua
...
--***					--和上面某部分相同就可以
						...
						if (Now_Time - Pre_Time >= 15*60) and floor(mod(nMoney / 31 * 17 + 73 + nLevel,21596+Quotiety)) == 15982 then
							...
							if ret > 0 then
								AddItemListToHuman(sceneId,selfId)
								BeginEvent(sceneId)
									strText = "你不讲价，运气好，一个洋参土鼈粥从天上掉进你的背包里。"
									AddText(sceneId,strText);
								EndEvent(sceneId)
								DispatchMissionTips(sceneId,selfId)
							else
...
--				DispatchMissionTips(sceneId,selfId)
			...
			--周活跃数据埋点
			LuaFnFinishZhouHuoYueMission(sceneId,selfId,25)
			--英雄大会埋点
			if LuaFnHaveImpactOfSpecificDataIndex(sceneId,selfId,10541) == 1 then
				AddHerosReturnsRecoed(sceneId,selfId,16)
			end
			--扣除官票
