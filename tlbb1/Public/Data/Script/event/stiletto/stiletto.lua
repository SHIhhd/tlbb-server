...
--前三孔打孔
--**********************************															
function x311200_StilettoEx_3( sceneId, selfId, idBagPos, idBagPosStuff )

	--可打孔的装备
	--0武器，1帽子，2衣服，3手套，4鞋
	--5腰带，6戒指，7项链，12护符，14护腕
	--15护肩
	local	tEquipGemTable	= { 0, 1, 2, 3, 4, 5, 6, 7, 12, 14, 15, 17, 18,HEQUIPEX_SHENBING }
...
--第四孔打孔
--**********************************								
function x311200_StilettoEx_4( sceneId, selfId, idBagPos, idBagPosStuff, type )	
	...
	for i = 1, getn(x311200_Stiletto_four_80_ID) do
	...
	--七情刃开放
	if LuaFnIsBagItemShenBing(sceneId,selfId,idBagPos) == 1 then
		istaozhuang = 1
	end
	
	--盟会筑器套装
	if LuaFnIsIdentityStilettoEquip(sceneId,selfId,idBagPos) then
		istaozhuang = 1
	end
...
--装备打孔材料购买
--**********************************
function x311200_StilettoEx_Yuanbao_Pay( sceneId, selfId, nItemId,nCheck )
	...
	local strTips = ScriptGlobal_Format("#{BSLCYH_130529_29}",
	...
	DispatchUICommand( sceneId, selfId, 2013060604 )
end
