	...
	if GetNumText() == 3   then
		...
		if ret == 1  then
			...
			if delret == 1  then
				...
				local str = format("#P #{_INFOUSR%s}经过一番努力，终于收集全了象徵七曜的金星、木星、水星、火星、土星、月亮、太阳七颗碎片。作为酬谢，大理的龚彩云特赠送给其一颗#{_INFOMSG%s}。",GetName(sceneId,selfId), transfer)
...
function x210242_PickupItem( sceneId, selfId, itemId, bagidx )
	...
	local str = format("#P #{_INFOUSR%s}刨出了一块怪石，擦乾净泥土之后，才发现居然是一块#{_INFOMSG%s}。",GetName(sceneId,selfId), transfer)
	BroadMsgByChatPipe(sceneId, selfId, str, 4)
