...
--任务文本描述
x311005_g_MissionName="平定叛乱"
x311005_g_MissionInfo="我们已经掌握了五台山叛军的全部底细，你去和陈永仁联系，杀死全部叛军，一个不留！"  --任务描述
x311005_g_MissionTarget="在刘健明的帮助下进入叛军营地，杀死1名叛军首领，13名叛军守卫，6名叛军门卫"	--任务目标
x311005_g_ContinueInfo="你要继续努力啊！"	--未完成任务的npc对话
x311005_g_MissionComplete="谢谢啊，俺们终于敢出门了"	--完成任务npc说话的话
...
x311005_g_LimitMembers=2 --可以进副本的最小队伍人数
x311005_g_TickTime=5 --回调脚本的时钟时间（单位：秒/次）
x311005_g_LimitTotalHoldTime=120 --副本可以存活的时间（单位：次数）
x311005_g_CloseTick=6	--副本关闭前倒计时（单位：次数）
x311005_g_NoUserTime=300 --副本中没有人后可以继续保存的时间（单位：秒）
...
function x311005_OnDefaultEvent( sceneId, selfId, targetId )
	if( IsHaveMission(sceneId,selfId,x311005_g_MissionId) > 0)  then	--如果玩家已经接了这个任务
		misIndex = GetMissionIndexByID(sceneId,selfId,x311005_g_MissionId)
		bDone = x311005_CheckSubmit( sceneId, selfId )
		if GetName(sceneId,targetId) == x311005_g_Name then		--是目标NPC
			BeginEvent(sceneId)
    ...
    elseif x311005_CheckAccept(sceneId,selfId) > 0 then		--没有任务，满足任务接收条件
...
function x311005_OnEnumerate( sceneId, selfId, targetId )
	...
	if IsHaveMission(sceneId,selfId,x311005_g_MissionId) > 0 then
		...
		elseif	GetName(sceneId,targetId) ~= x311005_g_Name	and	bDone==1 then --交任务的NPC, 任务完成
			AddNumText(sceneId, x311005_g_ScriptId,x311005_g_MissionName);
		end
	--满足任务接收条件
...
function x311005_OnAccept( sceneId, selfId )
	...
	if( IsHaveMission(sceneId,selfId,x311005_g_MissionId) > 0)  then	--已经有任务的
		misIndex = GetMissionIndexByID(sceneId,selfId,x311005_g_MissionId)
...
function x311005_MakeCopyScene( sceneId, selfId, nearmembercount )
	...
	LuaFnSetSceneLoad_Map(sceneId, "_pingpan_1.path"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
...
function x311005_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x311005_CheckSubmit( sceneId, selfId, selectRadioId )>0 then		--已经完成任务了
...
function x311005_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	if killednumber>=20 and IsKillOK==1 then
		...
		for i=0,num-1 do
			humanObjId = LuaFnGetCopyScene_HumanObjId(sceneId,i)	--取得当前场景里人的objId
			misIndex = GetMissionIndexByID(sceneId,humanObjId,x311005_g_MissionId)--取得任务数据索引值

			--将任务的第1号数据设置为1,表示完成的任务
			SetMissionByIndex(sceneId,humanObjId,misIndex,1,1)--设置任务数据

			BeginEvent(sceneId)
				strText = format("任务完成，将在%d秒后传送到入口位置", x311005_g_CloseTick*x311005_g_TickTime )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,humanObjId)
		end
...
function x311005_OnCopySceneTimer( sceneId, nowTime )
	...
	if leaveFlag == 1 then --需要离开
		...
		elseif leaveTickCount<x311005_g_CloseTick then
			...
			for	i=0,membercount-1 do
				mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
	  			BeginEvent(sceneId)
	  				strText = format("你将在%d秒后离开场景!", (x311005_g_CloseTick-leaveTickCount)*x311005_g_TickTime )
	  				AddText(sceneId,strText);
	  			EndEvent(sceneId)
	...
	elseif TickCount == x311005_g_LimitTotalHoldTime then --副本总时间限制到了
		...
		for	i=0,membercount-1 do
			mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
			DelMission( sceneId, mems[i], x311005_g_MissionId );--任务失败,删除之
