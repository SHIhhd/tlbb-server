...
function x800106_PetSavvy( sceneId, selfId, mainPetGuidH, mainPetGuidL )
	...
	--是否出战
	if mainPetGuidH==pgH and mainPetGuidL==pgL then
		x800106_NotifyTip(sceneId,selfId ,"#{ZSKJT_130428_23}")
		return
	end
	--是否加锁
	if LuaFnIsPetLockedByGUID(sceneId, selfId, mainPetGuidH, mainPetGuidL) == 1 then
		x800106_NotifyTip(sceneId,selfId ,"#{ZSKSSJ_081113_06}")
		return
	end
...
-- 宠物悟性快速提升
--**********************************
function x800106_QuickPetSavvy( sceneId, selfId, mainPetGuidH, mainPetGuidL )
	if GetLevel(sceneId,selfId) < 15 then
		x800106_NotifyTip(sceneId, selfId, "#{ZSKJT_130717_01}" );
		return
	end
	local pgH , pgL = LuaFnGetCurrentPetGUID(sceneId, selfId)
	--是否出战
	...
	local savvy = GetPetSavvy( sceneId, selfId, mainPetGuidH, mainPetGuidL )
	if savvy == 10 then
		x800106_NotifyTip(sceneId, selfId, "#{ZSKJT_130428_4}" );
		return
	end
	local nNeedYuanbao = x800106_g_YuanBaoCosts[savvy]
	local nCurYuanbao = YuanBao(sceneId,selfId,selfId,3,0)
	if nCurYuanbao < nNeedYuanbao then
		x800106_NotifyTip(sceneId, selfId, "#{ZSKJT_130428_6}" );
		return
	end
	if LuaFnIsCanYuanBaoCost(sceneId,selfId,nNeedYuanbao) ~= 1 then
		x800106_NotifyTip(sceneId, selfId, "#{HLYH_160120_34}" );
		return
	end
	if LuaFnCostYuanBao(sceneId,selfId,nNeedYuanbao,800106,"QuickPetSavvy",0,0) ~= 1 then
		return
	end
	--设置悟性
	SetPetSavvy( sceneId, selfId, mainPetGuidH, mainPetGuidL, 10 )
	Audit_PetImproveWuxing( sceneId, selfId, mainPetGuidH, mainPetGuidL, 1, 10 );
	
	x800106_NotifyTip(sceneId, selfId, "#{ZSKJT_130428_7}" );
	--成功的光效
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 18, 0);
	--ZSKJT_130717_02	#H#{_INFOUSR%s0}#H在#G苏州（171，238）#R云霏霏#H处得知#Y悟性快速提升#H之法，便催促其直接将#{_INFOMSG%s1}#H的悟性提升至#G10#H，珍兽资质大幅提升之余，避免了诸多麻烦，当真是一举数得！	
	local szPetTrans = LuaFnGetPetTransferByGUID( sceneId, selfId, mainPetGuidH, mainPetGuidL )
	local strText = ScriptGlobal_Format("#{ZSKJT_130717_02}",GetName(sceneId,selfId),szPetTrans)
