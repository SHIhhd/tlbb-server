...
function x800117_OnGemCarve( sceneId, selfId, GemItemPos, NeedItemPos,TargetID )
	...
	--扣除宝石和所需物品....
	if LuaFnIsItemAvailable( sceneId, selfId, GemItemPos ) ~= 1 then
		x800117_NotifyTip( sceneId, selfId, "宝石无法使用，宝石雕琢失败。" )
		return
	end
	if LuaFnIsItemAvailable( sceneId, selfId, NeedItemPos ) ~= 1 then
	...
	if ProductNeedBind == 1 then
		AddBindItem( sceneId,ProductID,1)
	else
		AddItem( sceneId,ProductID,1)
	end
	...
	--叠加改造
	--宝石是绑定的，优先扣除绑定
	if LuaFnGetItemBindStatus(sceneId,selfId,GemItemPos) == x800117_g_BINDED then
		if LuaFnDelBindAvailableItem(sceneId,selfId,GemItemID,1) ~= 1 then
			x800117_NotifyTip( sceneId, selfId, "宝石扣除失败，宝石雕琢失败。" )
			return
		end
	else
		if LuaFnDelNotBindAvailableItem(sceneId,selfId,GemItemID,1) ~= 1 then
			x800117_NotifyTip( sceneId, selfId, "宝石扣除失败，宝石雕琢失败。" )
			return
		end 
	end
	...
	if LuaFnGetItemBindStatus(sceneId,selfId,NeedItemPos) == 1  then
		LuaFnDelBindAvailableItem(sceneId,selfId,NeedItemID,1)
	else
		LuaFnDelNotBindAvailableItem(sceneId,selfId,NeedItemID,1)
	end

	--扣钱....
	if LuaFnCostMoneyWithPriority( sceneId, selfId, NeedMoney ) < 0 then
	...
	local GemItemInfo = LuaFnGetItemTransfer( sceneId, selfId, 0 )
