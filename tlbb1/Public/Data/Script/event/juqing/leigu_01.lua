...
x200040_g_CopySceneMap = "leigu_1.path"
...
function x200040_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "leigu_1.path"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
		...
		if nStep2 == 2  then
			if nowTime - nStartTime < 120  then
				if nowTime - nTime > 10  then
					...
					SetCharacterName(sceneId, nNpcId, "僵屍")
				...
				for ii=0, nMonsterNum-1 do
					...
					if GetName(sceneId, nMonsterId)  == "僵屍"  then
						-- 删除所有的僵尸
