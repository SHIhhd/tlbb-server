...
x200014_g_CopySceneMap = "yanzi.path"
...
function x200014_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "yanzi.path"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
		...
		if nNowTime - nPreTime >= 10 then
			...
			for ii=0, nMonsterNum-1 do
				...
				elseif GetName(sceneId, nMonsterId)  == "虎爪门弟子"  then
					nMonster[jj] = nMonsterId
					jj = jj+1
				elseif GetName(sceneId, nMonsterId)  == "虯髯武士"  then
					nMonster[jj] = nMonsterId
					jj = jj+1
			...
			if jj > 0  then
				...
				elseif GetName(sceneId, nSelMonster ) == "虯髯武士"    then
					x200014_DispatchMissionTipsToAll(sceneId, "王语嫣“虯髯武士的命门在膻中穴”")
