...
--任务文本描述
x226901_g_MissionName="桃花阵"
x226901_g_MissionInfo="#{MPFB_081023_04}"  --任务描述
x226901_g_MissionTarget="#{MPFB_081023_05}"	--任务目标
x226901_g_ContinueInfo="你要继续努力啊！"	--未完成任务的npc对话
x226901_g_MissionComplete="谢谢啊。"	--完成任务npc说话的话
...
--角色Mission变量说明
x226901_g_Param_huan		=0	--0号：已经完成的环数，在接收任务时候赋值
x226901_g_Param_ok			=1	--1号：当前任务是否完成(0未完成；1完成)
x226901_g_Param_sceneid		=2	--2号：当前副本任务的场景号
x226901_g_Param_teamid		=3	--3号：接副本任务时候的队伍号
x226901_g_Param_killcount	=4	--4号：杀死任务怪的数量
x226901_g_Param_time		=5	--5号：完成副本所用时间(单位：秒)
--6号：未用
--7号：未用

x226901_g_CopySceneType=FUBEN_TAOHUAZHEN	--副本类型，定义在ScriptGlobal.lua里面
x226901_g_LimitMembers=1			--可以进副本的最小队伍人数
x226901_g_TickTime=5				--回调脚本的时钟时间（单位：秒/次）
x226901_g_LimitTotalHoldTime=360	--副本可以存活的时间（单位：次数）,如果此时间到了，则任务将会失败
x226901_g_LimitTimeSuccess=500		--副本时间限制（单位：次数），如果此时间到了，任务完成
x226901_g_CloseTick=6				--副本关闭前倒计时（单位：次数）
x226901_g_NoUserTime=300			--副本中没有人后可以继续保存的时间（单位：秒）
x226901_g_DeadTrans=0				--死亡转移模式，0：死亡后还可以继续在副本，1：死亡后被强制移出副本
x226901_g_Fuben_X=26				--进入副本的位置X
x226901_g_Fuben_Z=46				--进入副本的位置Z
x226901_g_Back_X=97				--源场景位置X
x226901_g_Back_Z=74					--源场景位置Z
x226901_g_TotalNeedKill=10			--需要杀死怪物数量

--**********************************
--任务入口函数
--**********************************
function x226901_OnDefaultEvent( sceneId, selfId, targetId )
	if( IsHaveMission(sceneId,selfId,x226901_g_MissionId) > 0)  then	--如果玩家已经接了这个任务
  ...
  elseif x226901_CheckAccept(sceneId,selfId) > 0 then		--没有任务，满足任务接收条件
		--发送任务接受时显示的信息
		BeginEvent(sceneId)
			AddText(sceneId,x226901_g_MissionName)
...
function x226901_OnAccept( sceneId, selfId, targetId )
	if( IsHaveMission(sceneId,selfId,x226901_g_MissionId) > 0)  then	--已经有任务的
		misIndex = GetMissionIndexByID(sceneId,selfId,x226901_g_MissionId)
		...
		--加入任务到玩家列表
		if x226901_CheckAccept(sceneId,selfId) <= 0 then	--判断接收条件
			return 0
		end
...
function x226901_MakeCopyScene( sceneId, selfId)
	...
	LuaFnSetSceneLoad_Map(sceneId, "taohuazhen.path"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x226901_g_CopySceneType);--设置副本数据，这里将0号索引的数据设置为999，用于表示副本号999(数字自定义)
	LuaFnSetCopySceneData_Param(sceneId, 1, x226901_g_ScriptId);--将1号数据设置为副本场景事件脚本号
...
function x226901_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x226901_CheckSubmit( sceneId, selfId, selectRadioId )>0 then		--已经完成任务了
		--设置任务已经被完成过
		DelMission( sceneId,selfId,  x226901_g_MissionId )
...
function x226901_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	if killednumber<x226901_g_TotalNeedKill then
		...
		BeginEvent(sceneId)
			strText = format("已杀死怪物： %d/%d", killednumber, x226901_g_TotalNeedKill )
			AddText(sceneId,strText);
		EndEvent(sceneId)
		...
		for i=0,num-1 do
			...
			misIndex = GetMissionIndexByID(sceneId,humanObjId,x226901_g_MissionId) --取得任务数据索引值
			local killedcount = GetMissionParam( sceneId, humanObjId, misIndex, x226901_g_Param_killcount) --取得已经杀了的怪物数
			killedcount = killedcount +1 ;
			SetMissionByIndex(sceneId,humanObjId,misIndex,x226901_g_Param_killcount,killedcount) --设置任务数据
		end
	elseif killednumber>=x226901_g_TotalNeedKill then
		...
		for i=0,num-1 do
			...
			SetMissionByIndex(sceneId,humanObjId,misIndex,x226901_g_Param_ok,1)--设置任务数据
			--完成副本所用时间
			SetMissionByIndex(sceneId,humanObjId,misIndex,x226901_g_Param_time,TickCount*x226901_g_TickTime)--设置任务数据
...
function x226901_OnHumanDie( sceneId, selfId, killerId )
	if x226901_g_DeadTrans==1 then --死亡后需要被强制踢出场景

		misIndex = GetMissionIndexByID(sceneId,selfId,x226901_g_MissionId)--取得任务数据索引值

		--将任务的第1号数据设置为1,表示完成的任务
		SetMissionByIndex(sceneId,selfId,misIndex,x226901_g_Param_ok,1)--设置任务数据
		local TickCount = LuaFnGetCopySceneData_Param(sceneId, 2) ;--取得已经执行的定时次数
		--完成副本所用时间
		SetMissionByIndex(sceneId,selfId,misIndex,x226901_g_Param_time,TickCount*x226901_g_TickTime)--设置任务数据
...
function x226901_OnCopySceneTimer( sceneId, nowTime )
	...
	if leaveFlag == 1 then --需要离开
		...
		if leaveTickCount == x226901_g_CloseTick then --倒计时间到，大家都出去吧
		...
		elseif leaveTickCount<x226901_g_CloseTick then
			...
			for	i=0,membercount-1 do
				...
				if LuaFnIsObjValid(sceneId, mems[i]) == 1 and LuaFnIsCanDoScriptLogic(sceneId, mems[i]) == 1 then
	  			BeginEvent(sceneId)
	  				strText = format("你将在%d秒后离开场景!", (x226901_g_CloseTick-leaveTickCount)*x226901_g_TickTime )
	  				AddText(sceneId,strText);
	  			EndEvent(sceneId)
	...
	elseif TickCount == x226901_g_LimitTimeSuccess then
		...
		for	i=0,membercount-1 do
			...
			misIndex = GetMissionIndexByID(sceneId,mems[i],x226901_g_MissionId)--取得任务数据索引值
			--将任务的第1号数据设置为1,表示完成的任务
			SetMissionByIndex(sceneId,mems[i],misIndex,x226901_g_Param_ok,1)--设置任务数据
			--完成副本所用时间
			SetMissionByIndex(sceneId,mems[i],misIndex,x226901_g_Param_time,TickCount*x226901_g_TickTime)--设置任务数据
		end

		--设置副本关闭标志
		LuaFnSetCopySceneData_Param(sceneId, 4, 1) ;

	elseif TickCount == x226901_g_LimitTotalHoldTime then --副本总时间限制到了
		--此处设置副本任务有时间限制的情况，当时间到后处理...
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId)
		local mems = {}
		for	i=0,membercount-1 do
			mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
			DelMission( sceneId, mems[i], x226901_g_MissionId );--任务失败,删除之
			if LuaFnIsObjValid(sceneId, mems[i]) == 1 and LuaFnIsCanDoScriptLogic(sceneId, mems[i]) == 1 then
  			BeginEvent(sceneId)
