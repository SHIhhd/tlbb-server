...
function x402263_CheckCanEnter( sceneId, selfId, targetId )
	...
	for i=0, NearTeamSize-1 do
		...
		local PlayerId = GetNearTeamMember( sceneId, selfId, i )
		local lastDayCount = GetCampaignCountNum(sceneId,PlayerId,CAMPAIGN_PIAOMIAO)
...
--		end
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. "本日已经挑战过3次缥缈峰了。"
		return 0, msg
...
function x402263_OnPlayerEnter( sceneId, selfId )
	...
	local lastDayCount = GetCampaignCountNum(sceneId,selfId,CAMPAIGN_PIAOMIAO)
	if lastDayCount >= 3 then
		--直接踢出副本给这个卡副本的。
	...
	--设置挑战过一次缥缈峰....
	AddCampaignCountNum(sceneId,selfId,CAMPAIGN_PIAOMIAO)
...
-- 获取本副本的MonsterID
--**********************************
function x402263_GetFubenMonsterDataId( sceneId, selfId,nIndex)
	--nIndex 1-6
	--哈大霸 桑土公 乌老大 任平生 李秋水
	
	--李秋水特写
	if nIndex == 5 then
		return x402263_g_FightBOSSList[6],1
	end
	return x402263_g_FightBOSSList[nIndex],0
end
