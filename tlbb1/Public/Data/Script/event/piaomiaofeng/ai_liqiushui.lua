...
--免疫特定技能buff....
x402269_Buff_MianYi1	= 10472	--免疫一些负面效果....
x402269_Buff_MianYi2	= 10471	--免疫普通隐身....
...
--AI Index....
x402269_IDX_StopWatch						= 1	--秒表....
x402269_IDX_SkillA_CD						= 2	--A技能的CD时间....
x402269_IDX_SkillB_HPStep				= 3	--血量级别....
x402269_IDX_SkillB_Step					= 4	--B技能的Step....0=未发动 15=buff1 14=buff2 …… 1=buff15
x402269_IDX_SkillB_Type					= 5	--当前正在使用哪种类型的剑舞....
x402269_IDX_SkillC_CD						= 6	--C技能的CD时间....
x402269_IDX_SkillD_CD						= 7	--C技能的CD时间....
x402269_IDX_KuangBaoTimer				= 8	--狂暴的计时器....


x402269_IDX_CombatFlag 			= 1	--是否处于战斗状态的标志....
x402269_IDX_IsKuangBaoMode	= 2	--是否处于狂暴模式的标志....
...
function x402269_OnDie( sceneId, selfId, killerId )
	...
	for i=0, GetNearTeamCount( sceneId, playerID )-1  do
		...
		if LuaFnIsObjValid(sceneId,nPlayerId) == 1 and LuaFnIsCanDoScriptLogic(sceneId,nPlayerId) == 1 then
			--周活跃数据埋点
			LuaFnFinishZhouHuoYueMission(sceneId,nPlayerId,14)
			--英雄大会埋点
			if LuaFnHaveImpactOfSpecificDataIndex(sceneId,nPlayerId,10541) == 1 then
				AddHerosReturnsRecoed(sceneId,nPlayerId,12)
			end
			CallScriptFunction(SECKILL_SCRIPT,"OnFinish",sceneId, nPlayerId,CAMPAIGN_PIAOMIAO)
			--云侠赴战
			CallScriptFunction(890289,"OnFinish",sceneId, nPlayerId,CAMPAIGN_PIAOMIAO)
			--高光时刻结算
	...
	if playerName ~= nil then
		str = format("#{PMF_8812_06}#{_INFOUSR%s}#{PMF_8812_07}#{_INFOUSR%s}#{PMF_8812_08}", playerName, playerName); --李秋水
