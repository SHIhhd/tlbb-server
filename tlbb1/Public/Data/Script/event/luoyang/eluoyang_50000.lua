...
function x250000_OnMissionCheck( sceneId, selfId, npcid, scriptId, index1, index2, index3, indexpet )
  ...
  --indexpet 此索引为"绝对索引值"，即在0-5 这6个栏位中的序号。
	...
	if indexpet == 255 then --索引值返回255表示空，没提交珍兽
		...
		local nTodaySellCount = GetMissionDataEx(sceneId,selfId,MDEX_TODAY_SELL_PETCOUNT)
		if nTodaySellCount >= x250000_g_MaxSellCount then
			BeginEvent(sceneId)
				AddText(sceneId,"#{JJKHD_120808_103}");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		end
		...
		if ret1 > 0 then     --成功删除珍兽		    		    
			...
			SetMissionDataEx(sceneId,selfId,MDEX_TODAY_SELL_PETCOUNT,nTodaySellCount + 1)
		end
